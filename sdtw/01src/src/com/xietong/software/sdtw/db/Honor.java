...
@Description("志愿活动")
public class Honor implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "honor";
    private String TNAME = "honor";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","honorType","activityId","honorName","addtime","addperson","isEnable"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","honortype","activityid","honorname","addtime","addperson","isenable"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"HonorType","honortype","int"},{"ActivityId","activityid","int"},{"HonorName","honorname","String"},{"Addtime","addtime","java.util.Date"},{"Addperson","addperson","String"},{"IsEnable","isenable","int"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"honorType","honortype"},{"activityId","activityid"},{"honorName","honorname"},{"addtime","addtime"},{"addperson","addperson"},{"isEnable","isenable"}};
    ...
    // 荣誉分类
    protected int honorType = -1;
    public int getHonorType() { return honorType; }
    public void setHonorType(int aHonorType) { honorType = aHonorType; modifiedFields.add("HonorType"); }
    public void assignHonorType(int aHonorType) { honorType = aHonorType; }
    public void paramHonorType(HttpServletRequest request) { paramHonorType(request, "HonorType"); }
    public void paramHonorType(HttpServletRequest request, String name) { int tmp = ParamUtils.getIntParameter(request, name, getHonorType()); if (tmp != getHonorType()) { setHonorType(tmp); } }
    // 活动Id
    ...
    // 荣誉名称
    protected String honorName = "";
    public String getHonorName() {  return SqlTool.getDbString(honorName);  }
    public String getHonorName(int size) { return SqlTool.subString(getHonorName(), size); }
    public void setHonorName(String aHonorName) { honorName = aHonorName; modifiedFields.add("HonorName"); }
    public void assignHonorName(String aHonorName) { honorName = aHonorName; }
    public void paramHonorName(HttpServletRequest request) { paramHonorName(request, "HonorName"); }
    public void paramHonorName(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getHonorName()); if (!tmp.equals(getHonorName())) { setHonorName(tmp); } }
    // 添加时间
    ...
    public Honor (String db) {
        DBNAME = db;
    }
    public Honor (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public Honor (HttpServletRequest request) {
     ...
     * @return Honor类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public Honor getByParameter(HttpServletRequest request) {
        Honor v = new Honor(DBNAME, TNAME);
        v.assignParameter(request);
        return v;
    }
    public void assignParameter(HttpServletRequest request) {
        paramId(request);
        paramHonorType(request);
        paramActivityId(request);
        paramHonorName(request);
    ...
    */
    public void clone(Honor bean) {
        List ncl = Arrays.asList(noChangeList);
        if (!ncl.contains("id")) { setId(bean.getId()); }
        if (!ncl.contains("honorType")) { setHonorType(bean.getHonorType()); }
        if (!ncl.contains("activityId")) { setActivityId(bean.getActivityId()); }
        if (!ncl.contains("honorName")) { setHonorName(bean.getHonorName()); }
    ...
    public Honor getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        Honor[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                Honor ve = new Honor(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignHonorType(rs.getInt(2));
                ve.assignActivityId(rs.getInt(3));
                ve.assignHonorName(fromDbCh(rs.getString(4)));
                tmpDate = rs.getTimestamp(5); if (tmpDate != null) { ve.assignAddtime(new java.util.Date(tmpDate.getTime())); }
                ve.assignAddperson(fromDbCh(rs.getString(6)));
                ve.assignIsEnable(rs.getInt(7));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public Honor[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (Honor[])vrs.toArray(new Honor[vrs.size()]);
    }
    public Honor[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (honortype,activityid,honorname,addtime,addperson,isenable) VALUES (?,?,?,?,?,?)";
        ...
        try {
            ...
            pstmt.setInt(1, honorType);
            pstmt.setInt(2, activityId);
            pstmt.setString(3, toDbCh(honorName));
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,honortype,activityid,honorname,addtime,addperson,isenable) VALUES (?,?,?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            stmt = con.createStatement();
            pstmt.setInt(1, id);
            pstmt.setInt(2, honorType);
            pstmt.setInt(3, activityId);
            pstmt.setString(4, toDbCh(honorName));
            pstmt.setTimestamp(5, new java.sql.Timestamp(addtime.getTime()));
            pstmt.setString(6, toDbCh(addperson));
            pstmt.setInt(7, isEnable);
            iResult = pstmt.executeUpdate();
        }
        catch( SQLException sqle ) {
            sqle.printStackTrace();
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            Honor item = (Honor)it.next();
            Honor v = new Honor(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                int pi = 1;
                if (modifiedFields.contains("HonorType")) { pstmt.setInt(pi ++, honorType); }
                if (modifiedFields.contains("ActivityId")) { pstmt.setInt(pi ++, activityId); }
                if (modifiedFields.contains("HonorName")) { pstmt.setString(pi ++, toDbCh(honorName)); }
    ...
    */
    ...
    public Map toMap() {
        Map map = new HashMap();
        map.put("Id", getId() + "");
        map.put("HonorType", getHonorType() + "");
        map.put("ActivityId", getActivityId() + "");
        map.put("HonorName", getHonorName());
    ...
    public String toJsMap() {
        ...
        row.add("Id:\"" + getId() + "\"");
        row.add("HonorType:\"" + getHonorType() + "\"");
        row.add("ActivityId:\"" + getActivityId() + "\"");
        row.add("HonorName:\"" + Tool.jsSpecialChars(getHonorName()) + "\"");
    ...
    public int init() {
        ...
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0),honortype number(*,0),activityid number(*,0) not null,honorname varchar2(100),addtime date,addperson varchar2(45),isenable number(*,0))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        Honor v = new Honor(sourceDb, sourceTb);
    ...
    public int compareTo(Honor obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = ("" + getHonorType()).compareTo("" + obj.getHonorType()); if (rtn != 0) return rtn;
        rtn = ("" + getActivityId()).compareTo("" + obj.getActivityId()); if (rtn != 0) return rtn;
        rtn = getHonorName().compareTo(obj.getHonorName()); if (rtn != 0) return rtn;
