...
@Description("学生素质拓展培养认证单类型")
public class StudentRzdType implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "student_rzd_type";
    private String TNAME = "student_rzd_type";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","studentRzdType","name"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","studentrzdtype","name"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"StudentRzdType","studentrzdtype","int"},{"Name","name","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"studentRzdType","studentrzdtype"},{"name","name"}};
    ...
    public void setId(int aId) { id = aId; modifiedFields.add("Id"); refreshFlag = true; }
    ...
    // 类型
    protected int studentRzdType = -1;
    public int getStudentRzdType() { return studentRzdType; }
    public void setStudentRzdType(int aStudentRzdType) { studentRzdType = aStudentRzdType; modifiedFields.add("StudentRzdType"); }
    public void assignStudentRzdType(int aStudentRzdType) { studentRzdType = aStudentRzdType; }
    public void paramStudentRzdType(HttpServletRequest request) { paramStudentRzdType(request, "StudentRzdType"); }
    public void paramStudentRzdType(HttpServletRequest request, String name) { int tmp = ParamUtils.getIntParameter(request, name, getStudentRzdType()); if (tmp != getStudentRzdType()) { setStudentRzdType(tmp); } }
    // 名称
    ...
    public StudentRzdType (String db) {
        DBNAME = db;
    }
    public StudentRzdType (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public StudentRzdType (HttpServletRequest request) {
    ...
    public StudentRzdType () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return StudentRzdType类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public StudentRzdType getByParameter(HttpServletRequest request) {
        StudentRzdType v = new StudentRzdType(DBNAME, TNAME);
        v.assignParameter(request);
        return v;
    }
    public void assignParameter(HttpServletRequest request) {
        paramId(request);
        paramStudentRzdType(request);
        paramName(request);
    }
    /**
     * 将参数中类的实例数据复制到当前的类实例，noChangeList中包含属性名将不被复制。
    */
    public void clone(StudentRzdType bean) {
        List ncl = Arrays.asList(noChangeList);
        if (!ncl.contains("id")) { setId(bean.getId()); }
        if (!ncl.contains("studentRzdType")) { setStudentRzdType(bean.getStudentRzdType()); }
        if (!ncl.contains("name")) { setName(bean.getName()); }
    }
    public StudentRzdType getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        StudentRzdType[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                StudentRzdType ve = new StudentRzdType(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignStudentRzdType(rs.getInt(2));
    ...
    public StudentRzdType[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (StudentRzdType[])vrs.toArray(new StudentRzdType[vrs.size()]);
    }
    public StudentRzdType[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (studentrzdtype,name) VALUES (?,?)";
        ...
        CEditConst.setRefreshFlag(DBNAME, "com.xietong.software.sdtw.db.StudentRzdType");
        clearModifiedFields();
        return iResult;
    }
    /**
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,studentrzdtype,name) VALUES (?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            StudentRzdType item = (StudentRzdType)it.next();
            StudentRzdType v = new StudentRzdType(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                int pi = 1;
                if (modifiedFields.contains("StudentRzdType")) { pstmt.setInt(pi ++, studentRzdType); }
    ...
    */
    ...
    public Map toMap() {
        Map map = new HashMap();
        map.put("Id", getId() + "");
        map.put("StudentRzdType", getStudentRzdType() + "");
    ...
    public String toJsMap() {
        ...
        row.add("Id:\"" + getId() + "\"");
        row.add("StudentRzdType:\"" + getStudentRzdType() + "\"");
        row.add("Name:\"" + Tool.jsSpecialChars(getName()) + "\"");
        return "{" + Tool.join(",", row) + "}";
    }
    public int init() {
        List sqlList= new ArrayList();
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0),studentrzdtype number(*,0),name varchar2(45))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        StudentRzdType v = new StudentRzdType(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        StudentRzdType v = new StudentRzdType(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(StudentRzdType obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = ("" + getStudentRzdType()).compareTo("" + obj.getStudentRzdType()); if (rtn != 0) return rtn;
