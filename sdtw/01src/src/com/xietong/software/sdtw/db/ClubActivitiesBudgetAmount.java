...
@Description("社团活动信息")
public class ClubActivitiesBudgetAmount implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "club_activities_budgetamount";
    private String TNAME = "club_activities_budgetamount";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","purpose","price","num","amount","remark","cAId"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","purpose","price","num","amount","remark","caid"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Purpose","purpose","String"},{"Price","price","double"},{"Num","num","int"},{"Amount","amount","double"},{"Remark","remark","String"},{"CAId","caid","int"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"purpose","purpose"},{"price","price"},{"num","num"},{"amount","amount"},{"remark","remark"},{"cAId","caid"}};
    ...
    public String getPurpose() {  return SqlTool.getDbString(purpose);  }
    public String getPurpose(int size) { return SqlTool.subString(getPurpose(), size); }
    public void setPurpose(String aPurpose) { purpose = aPurpose; modifiedFields.add("Purpose"); }
    public void assignPurpose(String aPurpose) { purpose = aPurpose; }
    public void paramPurpose(HttpServletRequest request) { paramPurpose(request, "Purpose"); }
    public void paramPurpose(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getPurpose()); if (!tmp.equals(getPurpose())) { setPurpose(tmp); } }
    // 单价
    protected double price = 0.0;
    public double getPrice() { return price; }
    public void setPrice(double aPrice) { price = aPrice; modifiedFields.add("Price"); }
    public void assignPrice(double aPrice) { price = aPrice; }
    public void paramPrice(HttpServletRequest request) { paramPrice(request, "Price"); }
    public void paramPrice(HttpServletRequest request, String name) { double tmp = ParamUtils.getDoubleParameter(request, name, getPrice()); if (tmp != getPrice()) { setPrice(tmp); } }
    // 数量
    protected int num = -1;
    public int getNum() { return num; }
    public void setNum(int aNum) { num = aNum; modifiedFields.add("Num"); }
    public void assignNum(int aNum) { num = aNum; }
    public void paramNum(HttpServletRequest request) { paramNum(request, "Num"); }
    public void paramNum(HttpServletRequest request, String name) { int tmp = ParamUtils.getIntParameter(request, name, getNum()); if (tmp != getNum()) { setNum(tmp); } }
    // 金额
    protected double amount = 0.0;
    public double getAmount() { return amount; }
    public void setAmount(double aAmount) { amount = aAmount; modifiedFields.add("Amount"); }
    public void assignAmount(double aAmount) { amount = aAmount; }
    public void paramAmount(HttpServletRequest request) { paramAmount(request, "Amount"); }
    public void paramAmount(HttpServletRequest request, String name) { double tmp = ParamUtils.getDoubleParameter(request, name, getAmount()); if (tmp != getAmount()) { setAmount(tmp); } }
    // 备注
    ...
    public void setCAId(int aCAId) { cAId = aCAId; modifiedFields.add("CAId"); }
    public void assignCAId(int aCAId) { cAId = aCAId; }
    public void paramCAId(HttpServletRequest request) { paramCAId(request, "CAId"); }
    public void paramCAId(HttpServletRequest request, String name) { int tmp = ParamUtils.getIntParameter(request, name, getCAId()); if (tmp != getCAId()) { setCAId(tmp); } }
    ...
    public ClubActivitiesBudgetAmount (String db) {
        DBNAME = db;
    }
    public ClubActivitiesBudgetAmount (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public ClubActivitiesBudgetAmount (HttpServletRequest request) {
    ...
    public ClubActivitiesBudgetAmount () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return ClubActivitiesBudgetAmount类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public ClubActivitiesBudgetAmount getByParameter(HttpServletRequest request) {
        ClubActivitiesBudgetAmount v = new ClubActivitiesBudgetAmount(DBNAME, TNAME);
        v.assignParameter(request);
        return v;
    }
    public void assignParameter(HttpServletRequest request) {
        paramId(request);
        paramPurpose(request);
        paramPrice(request);
        paramNum(request);
        paramAmount(request);
        paramRemark(request);
    ...
    */
    public void clone(ClubActivitiesBudgetAmount bean) {
        List ncl = Arrays.asList(noChangeList);
        if (!ncl.contains("id")) { setId(bean.getId()); }
        if (!ncl.contains("purpose")) { setPurpose(bean.getPurpose()); }
        if (!ncl.contains("price")) { setPrice(bean.getPrice()); }
        if (!ncl.contains("num")) { setNum(bean.getNum()); }
        if (!ncl.contains("amount")) { setAmount(bean.getAmount()); }
        if (!ncl.contains("remark")) { setRemark(bean.getRemark()); }
        if (!ncl.contains("cAId")) { setCAId(bean.getCAId()); }
    }
    public ClubActivitiesBudgetAmount getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        ClubActivitiesBudgetAmount[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
     ...
     * @return ClubActivitiesBudgetAmount类的实例列表。
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                ClubActivitiesBudgetAmount ve = new ClubActivitiesBudgetAmount(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignPurpose(fromDbCh(rs.getString(2)));
                ve.assignPrice(rs.getDouble(3));
                ve.assignNum(rs.getInt(4));
                ve.assignAmount(rs.getDouble(5));
                ve.assignRemark(fromDbCh(rs.getString(6)));
                ve.assignCAId(rs.getInt(7));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public ClubActivitiesBudgetAmount[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (ClubActivitiesBudgetAmount[])vrs.toArray(new ClubActivitiesBudgetAmount[vrs.size()]);
    }
    public ClubActivitiesBudgetAmount[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (purpose,price,num,amount,remark,caid) VALUES (?,?,?,?,?,?)";
        ...
        try {
            ...
            pstmt.setString(1, toDbCh(purpose));
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,purpose,price,num,amount,remark,caid) VALUES (?,?,?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            stmt = con.createStatement();
            pstmt.setInt(1, id);
            pstmt.setString(2, toDbCh(purpose));
            pstmt.setDouble(3, price);
            pstmt.setInt(4, num);
            pstmt.setDouble(5, amount);
            pstmt.setString(6, toDbCh(remark));
            pstmt.setInt(7, cAId);
            iResult = pstmt.executeUpdate();
        }
        catch( SQLException sqle ) {
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            ClubActivitiesBudgetAmount item = (ClubActivitiesBudgetAmount)it.next();
            ClubActivitiesBudgetAmount v = new ClubActivitiesBudgetAmount(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                int pi = 1;
                if (modifiedFields.contains("Purpose")) { pstmt.setString(pi ++, toDbCh(purpose)); }
                if (modifiedFields.contains("Price")) { pstmt.setDouble(pi ++, price); }
                if (modifiedFields.contains("Num")) { pstmt.setInt(pi ++, num); }
                if (modifiedFields.contains("Amount")) { pstmt.setDouble(pi ++, amount); }
                if (modifiedFields.contains("Remark")) { pstmt.setString(pi ++, toDbCh(remark)); }
                if (modifiedFields.contains("CAId")) { pstmt.setInt(pi ++, cAId); }
                pstmt.setInt(pi, id);
                iResult = pstmt.executeUpdate();
            }
            catch( SQLException sqle ) {
    ...
    */
    ...
    public Map toMap() {
        ...
        map.put("CAId", getCAId() + "");
        return map;
    }
    public String toJsMap() {
        List row = new ArrayList();
        row.add("Id:\"" + getId() + "\"");
        row.add("Purpose:\"" + Tool.jsSpecialChars(getPurpose()) + "\"");
        row.add("Price:\"" + getPrice() + "\"");
        row.add("Num:\"" + getNum() + "\"");
        row.add("Amount:\"" + getAmount() + "\"");
        row.add("Remark:\"" + Tool.jsSpecialChars(getRemark()) + "\"");
        row.add("CAId:\"" + getCAId() + "\"");
        return "{" + Tool.join(",", row) + "}";
    }
    public int init() {
        List sqlList= new ArrayList();
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,purpose varchar2(100),price number(*,2),num number(*,0),amount number(*,2),remark varchar2(2000),caid number(*,0))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        ClubActivitiesBudgetAmount v = new ClubActivitiesBudgetAmount(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        ClubActivitiesBudgetAmount v = new ClubActivitiesBudgetAmount(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(ClubActivitiesBudgetAmount obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = getPurpose().compareTo(obj.getPurpose()); if (rtn != 0) return rtn;
        rtn = ("" + getPrice()).compareTo("" + obj.getPrice()); if (rtn != 0) return rtn;
        rtn = ("" + getNum()).compareTo("" + obj.getNum()); if (rtn != 0) return rtn;
        rtn = ("" + getAmount()).compareTo("" + obj.getAmount()); if (rtn != 0) return rtn;
        rtn = getRemark().compareTo(obj.getRemark()); if (rtn != 0) return rtn;
        rtn = ("" + getCAId()).compareTo("" + obj.getCAId()); if (rtn != 0) return rtn;
        return rtn;
    }
}
