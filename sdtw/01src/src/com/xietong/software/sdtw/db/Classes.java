...
@Description("年级")
public class Classes implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "classes";
    private String TNAME = "classes";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","academy","name","gid","opName","beizhu","isEnable"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","academy","name","gid","opname","beizhu","isenable"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Academy","academy","int"},{"Name","name","String"},{"Gid","gid","int"},{"OpName","opname","int"},{"Beizhu","beizhu","String"},{"IsEnable","isenable","int"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"academy","academy"},{"name","name"},{"gid","gid"},{"opName","opname"},{"beizhu","beizhu"},{"isEnable","isenable"}};
    ...
    public void setGid(int aGid) { gid = aGid; modifiedFields.add("Gid"); }
    public void assignGid(int aGid) { gid = aGid; }
    public void paramGid(HttpServletRequest request) { paramGid(request, "Gid"); }
    public void paramGid(HttpServletRequest request, String name) { int tmp = ParamUtils.getIntParameter(request, name, getGid()); if (tmp != getGid()) { setGid(tmp); } }
    // 操作人
    ...
    public Classes (String db) {
        DBNAME = db;
    }
    public Classes (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public Classes (HttpServletRequest request) {
     ...
     * @return Classes类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public Classes getByParameter(HttpServletRequest request) {
        Classes v = new Classes(DBNAME, TNAME);
    ...
    */
    public void clone(Classes bean) {
        ...
        if (!ncl.contains("gid")) { setGid(bean.getGid()); }
    ...
    public Classes getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        Classes[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                Classes ve = new Classes(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignAcademy(rs.getInt(2));
                ve.assignName(fromDbCh(rs.getString(3)));
                ve.assignGid(rs.getInt(4));
                ve.assignOpName(rs.getInt(5));
                ve.assignBeizhu(fromDbCh(rs.getString(6)));
    ...
    public Classes[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (Classes[])vrs.toArray(new Classes[vrs.size()]);
    }
    public Classes[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (academy,name,gid,opname,beizhu,isenable) VALUES (?,?,?,?,?,?)";
        ...
        CEditConst.setRefreshFlag(DBNAME, "com.xietong.software.sdtw.db.Classes");
        clearModifiedFields();
        return iResult;
    }
    /**
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,academy,name,gid,opname,beizhu,isenable) VALUES (?,?,?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            stmt = con.createStatement();
            pstmt.setInt(1, id);
            pstmt.setInt(2, academy);
            pstmt.setString(3, toDbCh(name));
            pstmt.setInt(4, gid);
            pstmt.setInt(5, opName);
            pstmt.setString(6, toDbCh(beizhu));
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            Classes item = (Classes)it.next();
            Classes v = new Classes(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                if (modifiedFields.contains("Gid")) { pstmt.setInt(pi ++, gid); }
    ...
    */
    ...
    public int init() {
        ...
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,academy number(*,0),name varchar2(50),gid number(*,0),opname number(*,0),beizhu varchar2(500),isenable number(*,0))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        Classes v = new Classes(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        Classes v = new Classes(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(Classes obj) {
        int rtn = 0;
        ...
        rtn = ("" + getGid()).compareTo("" + obj.getGid()); if (rtn != 0) return rtn;
