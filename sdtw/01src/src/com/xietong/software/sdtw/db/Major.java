...
@Description("专业表")
public class Major implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "major";
    private String TNAME = "major";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","name","academy","pid","code"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","name","academy","pid","code"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Name","name","String"},{"Academy","academy","int"},{"Pid","pid","int"},{"Code","code","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"name","name"},{"academy","academy"},{"pid","pid"},{"code","code"}};
    ...
    public Major (String db) {
        DBNAME = db;
    }
    public Major (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
     ...
     * @return Major类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public Major getByParameter(HttpServletRequest request) {
        Major v = new Major(DBNAME, TNAME);
    ...
    */
    ...
    public Major getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        Major[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                Major ve = new Major(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignName(fromDbCh(rs.getString(2)));
                ve.assignAcademy(rs.getInt(3));
                ve.assignPid(rs.getInt(4));
                ve.assignCode(fromDbCh(rs.getString(5)));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public Major[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (Major[])vrs.toArray(new Major[vrs.size()]);
    }
    public Major[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (name,academy,pid,code) VALUES (?,?,?,?)";
        ...
        CEditConst.setRefreshFlag(DBNAME, "com.xietong.software.sdtw.db.Major");
        clearModifiedFields();
        return iResult;
    }
    /**
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,name,academy,pid,code) VALUES (?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            pstmt.setInt(3, academy);
            pstmt.setInt(4, pid);
            pstmt.setString(5, toDbCh(code));
            iResult = pstmt.executeUpdate();
        }
        catch( SQLException sqle ) {
            sqle.printStackTrace();
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            Major item = (Major)it.next();
            Major v = new Major(dbname);
    ...
    */
    ...
    public TreeItem cloneTi(Major item) {
    ...
    public TreeItem initTree(Major item, Map parent) {
    ...
    public int init() {
        ...
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,name varchar2(45),academy number(*,0),pid number(*,0),code varchar2(45))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        Major v = new Major(sourceDb, sourceTb);
