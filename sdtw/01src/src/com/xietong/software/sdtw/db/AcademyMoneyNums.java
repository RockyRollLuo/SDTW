...
public class AcademyMoneyNums implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    public AcademyMoneyNums (String db) {
        DBNAME = db;
    }
    public AcademyMoneyNums (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public AcademyMoneyNums (HttpServletRequest request) {
    ...
    public AcademyMoneyNums () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return AcademyMoneyNums类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public AcademyMoneyNums getByParameter(HttpServletRequest request) {
        AcademyMoneyNums v = new AcademyMoneyNums(DBNAME, TNAME);
    ...
    */
    public void clone(AcademyMoneyNums bean) {
    ...
    public AcademyMoneyNums getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        AcademyMoneyNums[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                AcademyMoneyNums ve = new AcademyMoneyNums(DBNAME, TNAME);
    ...
    public AcademyMoneyNums[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (AcademyMoneyNums[])vrs.toArray(new AcademyMoneyNums[vrs.size()]);
    }
    public AcademyMoneyNums[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            AcademyMoneyNums item = (AcademyMoneyNums)it.next();
            AcademyMoneyNums v = new AcademyMoneyNums(dbname);
    ...
    */
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        AcademyMoneyNums v = new AcademyMoneyNums(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        AcademyMoneyNums v = new AcademyMoneyNums(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(AcademyMoneyNums obj) {
        int rtn = 0;
