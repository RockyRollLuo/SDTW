...
@Description("山大创库通知")
public class ChuangKu implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "chuangku";
    private String TNAME = "chuangku";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","ck2","ck1","addTime","ck3"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","ck2","ck1","addtime","ck3"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Ck2","ck2","java.util.Date"},{"Ck1","ck1","String"},{"AddTime","addtime","String"},{"Ck3","ck3","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"ck2","ck2"},{"ck1","ck1"},{"addTime","addtime"},{"ck3","ck3"}};
    ...
    // 数据表中所有日期类型的字段
    private static final String[] _dateFields = {"CK2"};
    ...
    // 起止日期
    protected java.util.Date ck2 = java.sql.Date.valueOf("0001-01-01");
    public java.util.Date getCk2() { return ck2; }
    public void setCk2(java.util.Date aCk2) { ck2 = aCk2; modifiedFields.add("Ck2"); }
    public void assignCk2(java.util.Date aCk2) { ck2 = aCk2; }
    public void paramCk2(HttpServletRequest request) { paramCk2(request, "Ck2"); }
    public void paramCk2(HttpServletRequest request, String name) { java.util.Date tmp = ParamUtils.getDateTimeParameter(request, name, getCk2()); if (tmp.compareTo(getCk2()) != 0) { setCk2(tmp); } }
    // 通知名称
    protected String ck1 = "";
    public String getCk1() {  return SqlTool.getDbString(ck1);  }
    public String getCk1(int size) { return SqlTool.subString(getCk1(), size); }
    public void setCk1(String aCk1) { ck1 = aCk1; modifiedFields.add("Ck1"); }
    public void assignCk1(String aCk1) { ck1 = aCk1; }
    public void paramCk1(HttpServletRequest request) { paramCk1(request, "Ck1"); }
    public void paramCk1(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getCk1()); if (!tmp.equals(getCk1())) { setCk1(tmp); } }
    // 添加时间
    ...
    public String getCk3() {  return SqlTool.getDbString(ck3);  }
    public String getCk3(int size) { return SqlTool.subString(getCk3(), size); }
    public void setCk3(String aCk3) { ck3 = aCk3; modifiedFields.add("Ck3"); }
    public void assignCk3(String aCk3) { ck3 = aCk3; }
    public void paramCk3(HttpServletRequest request) { paramCk3(request, "Ck3"); }
    public void paramCk3(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getCk3()); if (!tmp.equals(getCk3())) { setCk3(tmp); } }
    ...
    public ChuangKu (String db) {
        DBNAME = db;
    }
    public ChuangKu (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public ChuangKu (HttpServletRequest request) {
     ...
     * @return ChuangKu类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public ChuangKu getByParameter(HttpServletRequest request) {
        ChuangKu v = new ChuangKu(DBNAME, TNAME);
    ...
    */
    public void clone(ChuangKu bean) {
        List ncl = Arrays.asList(noChangeList);
        if (!ncl.contains("id")) { setId(bean.getId()); }
        if (!ncl.contains("ck2")) { setCk2(bean.getCk2()); }
        if (!ncl.contains("ck1")) { setCk1(bean.getCk1()); }
        if (!ncl.contains("addTime")) { setAddTime(bean.getAddTime()); }
        if (!ncl.contains("ck3")) { setCk3(bean.getCk3()); }
    }
    public ChuangKu getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        ChuangKu[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                ChuangKu ve = new ChuangKu(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                tmpDate = rs.getTimestamp(2); if (tmpDate != null) { ve.assignCk2(new java.util.Date(tmpDate.getTime())); }
                ve.assignCk1(fromDbCh(rs.getString(3)));
                ve.assignAddTime(fromDbCh(rs.getString(4)));
                ve.assignCk3(fromDbCh(rs.getString(5)));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public ChuangKu[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (ChuangKu[])vrs.toArray(new ChuangKu[vrs.size()]);
    }
    public ChuangKu[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (ck2,ck1,addtime,ck3) VALUES (?,?,?,?)";
        ...
        try {
            ...
            pstmt.setTimestamp(1, new java.sql.Timestamp(ck2.getTime()));
            pstmt.setString(2, toDbCh(ck1));
            pstmt.setString(3, toDbCh(addTime));
            pstmt.setString(4, toDbCh(ck3));
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,ck2,ck1,addtime,ck3) VALUES (?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            stmt = con.createStatement();
            pstmt.setInt(1, id);
            pstmt.setTimestamp(2, new java.sql.Timestamp(ck2.getTime()));
            pstmt.setString(3, toDbCh(ck1));
            pstmt.setString(4, toDbCh(addTime));
            pstmt.setString(5, toDbCh(ck3));
            iResult = pstmt.executeUpdate();
        }
        catch( SQLException sqle ) {
            sqle.printStackTrace();
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            ChuangKu item = (ChuangKu)it.next();
            ChuangKu v = new ChuangKu(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                int pi = 1;
                if (modifiedFields.contains("Ck2")) { pstmt.setTimestamp(pi ++, new java.sql.Timestamp(ck2.getTime())); }
                if (modifiedFields.contains("Ck1")) { pstmt.setString(pi ++, toDbCh(ck1)); }
                if (modifiedFields.contains("AddTime")) { pstmt.setString(pi ++, toDbCh(addTime)); }
                if (modifiedFields.contains("Ck3")) { pstmt.setString(pi ++, toDbCh(ck3)); }
                pstmt.setInt(pi, id);
                iResult = pstmt.executeUpdate();
            }
            catch( SQLException sqle ) {
    ...
    */
    ...
    public Map toMap() {
        Map map = new HashMap();
        map.put("Id", getId() + "");
        map.put("Ck2", Tool.stringOfDateTime(getCk2()));
        map.put("Ck1", getCk1());
    ...
    public String toJsMap() {
        ...
        row.add("Id:\"" + getId() + "\"");
        row.add("Ck2:\"" + Tool.stringOfDateTime(getCk2()) + "\"");
        row.add("Ck1:\"" + Tool.jsSpecialChars(getCk1()) + "\"");
        row.add("AddTime:\"" + Tool.jsSpecialChars(getAddTime()) + "\"");
        row.add("Ck3:\"" + Tool.jsSpecialChars(getCk3()) + "\"");
        return "{" + Tool.join(",", row) + "}";
    }
    public int init() {
        List sqlList= new ArrayList();
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,ck2 date,ck1 varchar2(50),addtime varchar2(50),ck3 varchar2(200))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        ChuangKu v = new ChuangKu(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        ChuangKu v = new ChuangKu(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(ChuangKu obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = getCk2().compareTo(obj.getCk2()); if (rtn != 0) return rtn;
        rtn = getCk1().compareTo(obj.getCk1()); if (rtn != 0) return rtn;
        rtn = getAddTime().compareTo(obj.getAddTime()); if (rtn != 0) return rtn;
        rtn = getCk3().compareTo(obj.getCk3()); if (rtn != 0) return rtn;
        return rtn;
    }
}
