...
@Description("日志")
public class Logs365days implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "logs365days";
    private String TNAME = "logs365days";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","userAccount","ipAddress","loginTime","userRole","behavior"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","useraccount","ipaddress","logintime","userrole","behavior"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"UserAccount","useraccount","String"},{"IpAddress","ipaddress","String"},{"LoginTime","logintime","String"},{"UserRole","userrole","String"},{"Behavior","behavior","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"userAccount","useraccount"},{"ipAddress","ipaddress"},{"loginTime","logintime"},{"userRole","userrole"},{"behavior","behavior"}};
    ...
    public String getIpAddress() {  return SqlTool.getDbString(ipAddress);  }
    public String getIpAddress(int size) { return SqlTool.subString(getIpAddress(), size); }
    public void setIpAddress(String aIpAddress) { ipAddress = aIpAddress; modifiedFields.add("IpAddress"); }
    public void assignIpAddress(String aIpAddress) { ipAddress = aIpAddress; }
    public void paramIpAddress(HttpServletRequest request) { paramIpAddress(request, "IpAddress"); }
    public void paramIpAddress(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getIpAddress()); if (!tmp.equals(getIpAddress())) { setIpAddress(tmp); } }
    // 登录时间
    protected String loginTime = "";
    public String getLoginTime() {  return SqlTool.getDbString(loginTime);  }
    public String getLoginTime(int size) { return SqlTool.subString(getLoginTime(), size); }
    public void setLoginTime(String aLoginTime) { loginTime = aLoginTime; modifiedFields.add("LoginTime"); }
    public void assignLoginTime(String aLoginTime) { loginTime = aLoginTime; }
    public void paramLoginTime(HttpServletRequest request) { paramLoginTime(request, "LoginTime"); }
    public void paramLoginTime(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getLoginTime()); if (!tmp.equals(getLoginTime())) { setLoginTime(tmp); } }
    // 用户权限
    ...
    public String getBehavior() {  return SqlTool.getDbString(behavior);  }
    public String getBehavior(int size) { return SqlTool.subString(getBehavior(), size); }
    public void setBehavior(String aBehavior) { behavior = aBehavior; modifiedFields.add("Behavior"); }
    public void assignBehavior(String aBehavior) { behavior = aBehavior; }
    public void paramBehavior(HttpServletRequest request) { paramBehavior(request, "Behavior"); }
    public void paramBehavior(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getBehavior()); if (!tmp.equals(getBehavior())) { setBehavior(tmp); } }
    ...
    public Logs365days (String db) {
        DBNAME = db;
    }
    public Logs365days (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public Logs365days (HttpServletRequest request) {
     ...
     * @return Logs365days类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public Logs365days getByParameter(HttpServletRequest request) {
        Logs365days v = new Logs365days(DBNAME, TNAME);
    ...
    public void assignParameter(HttpServletRequest request) {
        ...
        paramIpAddress(request);
        paramLoginTime(request);
        paramUserRole(request);
        paramBehavior(request);
    }
    /**
     * 将参数中类的实例数据复制到当前的类实例，noChangeList中包含属性名将不被复制。
    */
    public void clone(Logs365days bean) {
        ...
        if (!ncl.contains("ipAddress")) { setIpAddress(bean.getIpAddress()); }
        if (!ncl.contains("loginTime")) { setLoginTime(bean.getLoginTime()); }
        if (!ncl.contains("userRole")) { setUserRole(bean.getUserRole()); }
        if (!ncl.contains("behavior")) { setBehavior(bean.getBehavior()); }
    }
    public Logs365days getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        Logs365days[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                Logs365days ve = new Logs365days(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignUserAccount(fromDbCh(rs.getString(2)));
                ve.assignIpAddress(fromDbCh(rs.getString(3)));
                ve.assignLoginTime(fromDbCh(rs.getString(4)));
                ve.assignUserRole(fromDbCh(rs.getString(5)));
                ve.assignBehavior(fromDbCh(rs.getString(6)));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public Logs365days[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (Logs365days[])vrs.toArray(new Logs365days[vrs.size()]);
    }
    public Logs365days[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (useraccount,ipaddress,logintime,userrole,behavior) VALUES (?,?,?,?,?)";
        ...
        try {
            ...
            pstmt.setString(2, toDbCh(ipAddress));
            pstmt.setString(3, toDbCh(loginTime));
            pstmt.setString(4, toDbCh(userRole));
            pstmt.setString(5, toDbCh(behavior));
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,useraccount,ipaddress,logintime,userrole,behavior) VALUES (?,?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            pstmt.setString(3, toDbCh(ipAddress));
            pstmt.setString(4, toDbCh(loginTime));
            pstmt.setString(5, toDbCh(userRole));
            pstmt.setString(6, toDbCh(behavior));
            iResult = pstmt.executeUpdate();
        }
        catch( SQLException sqle ) {
            sqle.printStackTrace();
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            Logs365days item = (Logs365days)it.next();
            Logs365days v = new Logs365days(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                if (modifiedFields.contains("IpAddress")) { pstmt.setString(pi ++, toDbCh(ipAddress)); }
                if (modifiedFields.contains("LoginTime")) { pstmt.setString(pi ++, toDbCh(loginTime)); }
                if (modifiedFields.contains("UserRole")) { pstmt.setString(pi ++, toDbCh(userRole)); }
                if (modifiedFields.contains("Behavior")) { pstmt.setString(pi ++, toDbCh(behavior)); }
                pstmt.setInt(pi, id);
                iResult = pstmt.executeUpdate();
            }
            catch( SQLException sqle ) {
    ...
    */
    ...
    public String toJsMap() {
        ...
        row.add("IpAddress:\"" + Tool.jsSpecialChars(getIpAddress()) + "\"");
        row.add("LoginTime:\"" + Tool.jsSpecialChars(getLoginTime()) + "\"");
        row.add("UserRole:\"" + Tool.jsSpecialChars(getUserRole()) + "\"");
        row.add("Behavior:\"" + Tool.jsSpecialChars(getBehavior()) + "\"");
        return "{" + Tool.join(",", row) + "}";
    }
    public int init() {
        List sqlList= new ArrayList();
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,useraccount varchar2(45) not null,ipaddress varchar2(45) not null,logintime varchar2(45) not null,userrole varchar2(45) not null,behavior varchar2(45) not null)"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        Logs365days v = new Logs365days(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        Logs365days v = new Logs365days(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(Logs365days obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = getUserAccount().compareTo(obj.getUserAccount()); if (rtn != 0) return rtn;
        rtn = getIpAddress().compareTo(obj.getIpAddress()); if (rtn != 0) return rtn;
        rtn = getLoginTime().compareTo(obj.getLoginTime()); if (rtn != 0) return rtn;
        rtn = getUserRole().compareTo(obj.getUserRole()); if (rtn != 0) return rtn;
        rtn = getBehavior().compareTo(obj.getBehavior()); if (rtn != 0) return rtn;
        return rtn;
    }
}
