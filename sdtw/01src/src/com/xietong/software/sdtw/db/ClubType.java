...
@Description("社团类别")
public class ClubType implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    private static Log log = LogFactory.getLog(ClubType.class);
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "clubtype";
    private String TNAME = "clubtype";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","name","isEnable"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","name","isenable"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Name","name","String"},{"IsEnable","isenable","int"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"name","name"},{"isEnable","isenable"}};
    ...
    public ClubType (String db) {
        DBNAME = db;
    }
    public ClubType (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public ClubType (HttpServletRequest request) {
     ...
     * @return ClubType类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public ClubType getByParameter(HttpServletRequest request) {
        ClubType v = new ClubType(DBNAME, TNAME);
    ...
    */
    public void clone(ClubType bean) {
    ...
    public ClubType getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        ClubType[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                ClubType ve = new ClubType(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignName(fromDbCh(rs.getString(2)));
                ve.assignIsEnable(rs.getInt(3));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public ClubType[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (ClubType[])vrs.toArray(new ClubType[vrs.size()]);
    }
    public ClubType[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (name,isenable) VALUES (?,?)";
        ...
        CEditConst.setRefreshFlag(DBNAME, "com.xietong.software.sdtw.db.ClubType");
        clearModifiedFields();
        return iResult;
    }
    /**
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,name,isenable) VALUES (?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            ClubType item = (ClubType)it.next();
            ClubType v = new ClubType(dbname);
    ...
    */
    ...
    public int init() {
        ...
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0),name varchar2(45),isenable number(*,0))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        ClubType v = new ClubType(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        ClubType v = new ClubType(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(ClubType obj) {
        int rtn = 0;
