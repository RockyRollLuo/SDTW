...
@Description("通讯录")
public class AddBookGroups implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    private static Log log = LogFactory.getLog(AddBookGroups.class);
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "addbookgroups";
    private String TNAME = "addbookgroups";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","userAccount","addTime","groupList"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","useraccount","addtime","grouplist"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"UserAccount","useraccount","String"},{"AddTime","addtime","String"},{"GroupList","grouplist","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"userAccount","useraccount"},{"addTime","addtime"},{"groupList","grouplist"}};
    ...
    public String getGroupList() {  return SqlTool.getDbString(groupList);  }
    public String getGroupList(int size) { return SqlTool.subString(getGroupList(), size); }
    public void setGroupList(String aGroupList) { groupList = aGroupList; modifiedFields.add("GroupList"); }
    public void assignGroupList(String aGroupList) { groupList = aGroupList; }
    public void paramGroupList(HttpServletRequest request) { paramGroupList(request, "GroupList"); }
    public void paramGroupList(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getGroupList()); if (!tmp.equals(getGroupList())) { setGroupList(tmp); } }
    ...
    public AddBookGroups (String db) {
        DBNAME = db;
    }
    public AddBookGroups (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public AddBookGroups (HttpServletRequest request) {
    ...
    public AddBookGroups () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return AddBookGroups类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public AddBookGroups getByParameter(HttpServletRequest request) {
        AddBookGroups v = new AddBookGroups(DBNAME, TNAME);
    ...
    public void assignParameter(HttpServletRequest request) {
        ...
        paramGroupList(request);
    }
    /**
     * 将参数中类的实例数据复制到当前的类实例，noChangeList中包含属性名将不被复制。
    */
    public void clone(AddBookGroups bean) {
        ...
        if (!ncl.contains("groupList")) { setGroupList(bean.getGroupList()); }
    }
    public AddBookGroups getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        AddBookGroups[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                AddBookGroups ve = new AddBookGroups(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignUserAccount(fromDbCh(rs.getString(2)));
                ve.assignAddTime(fromDbCh(rs.getString(3)));
                ve.assignGroupList(fromDbCh(rs.getString(4)));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public AddBookGroups[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (AddBookGroups[])vrs.toArray(new AddBookGroups[vrs.size()]);
    }
    public AddBookGroups[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (useraccount,addtime,grouplist) VALUES (?,?,?)";
        ...
        try {
            ...
            pstmt.setString(2, toDbCh(addTime));
            pstmt.setString(3, toDbCh(groupList));
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,useraccount,addtime,grouplist) VALUES (?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            pstmt.setString(4, toDbCh(groupList));
            iResult = pstmt.executeUpdate();
        }
        catch( SQLException sqle ) {
            sqle.printStackTrace();
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            AddBookGroups item = (AddBookGroups)it.next();
            AddBookGroups v = new AddBookGroups(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                if (modifiedFields.contains("GroupList")) { pstmt.setString(pi ++, toDbCh(groupList)); }
                pstmt.setInt(pi, id);
                iResult = pstmt.executeUpdate();
            }
            catch( SQLException sqle ) {
    ...
    */
    ...
    public Map toMap() {
        ...
        map.put("GroupList", getGroupList());
        return map;
    }
    public String toJsMap() {
        ...
        row.add("GroupList:\"" + Tool.jsSpecialChars(getGroupList()) + "\"");
        return "{" + Tool.join(",", row) + "}";
    }
    public int init() {
        List sqlList= new ArrayList();
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,useraccount varchar2(45),addtime varchar2(45),grouplist varchar2(400))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        AddBookGroups v = new AddBookGroups(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        AddBookGroups v = new AddBookGroups(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(AddBookGroups obj) {
        int rtn = 0;
        ...
        rtn = getGroupList().compareTo(obj.getGroupList()); if (rtn != 0) return rtn;
        return rtn;
    }
}
