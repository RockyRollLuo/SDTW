...
@Description("山大志库")
public class VolunteerZhikuDecalre implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "volunteer_zhiku_decalre";
    private String TNAME = "volunteer_zhiku_decalre";
    ...
    public VolunteerZhikuDecalre (String db) {
        DBNAME = db;
    }
    public VolunteerZhikuDecalre (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public VolunteerZhikuDecalre (HttpServletRequest request) {
    ...
    public VolunteerZhikuDecalre () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return VolunteerZhikuDecalre类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public VolunteerZhikuDecalre getByParameter(HttpServletRequest request) {
        VolunteerZhikuDecalre v = new VolunteerZhikuDecalre(DBNAME, TNAME);
    ...
    */
    public void clone(VolunteerZhikuDecalre bean) {
    ...
    public VolunteerZhikuDecalre getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        VolunteerZhikuDecalre[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
     ...
     * @return VolunteerZhikuDecalre类的实例列表。
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                VolunteerZhikuDecalre ve = new VolunteerZhikuDecalre(DBNAME, TNAME);
    ...
    public VolunteerZhikuDecalre[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (VolunteerZhikuDecalre[])vrs.toArray(new VolunteerZhikuDecalre[vrs.size()]);
    }
    public VolunteerZhikuDecalre[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            VolunteerZhikuDecalre item = (VolunteerZhikuDecalre)it.next();
            VolunteerZhikuDecalre v = new VolunteerZhikuDecalre(dbname);
    ...
    */
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        VolunteerZhikuDecalre v = new VolunteerZhikuDecalre(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        VolunteerZhikuDecalre v = new VolunteerZhikuDecalre(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(VolunteerZhikuDecalre obj) {
        int rtn = 0;
