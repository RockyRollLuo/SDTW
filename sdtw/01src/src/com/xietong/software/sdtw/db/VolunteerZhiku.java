...
@Description("山大志库")
public class VolunteerZhiku implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "volunteer_zhiku";
    private String TNAME = "volunteer_zhiku";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","noticeName","startdate","enddate","noticeInfo"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","noticename","startdate","enddate","noticeinfo"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"NoticeName","noticename","String"},{"Startdate","startdate","java.util.Date"},{"Enddate","enddate","java.util.Date"},{"NoticeInfo","noticeinfo","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"noticeName","noticename"},{"startdate","startdate"},{"enddate","enddate"},{"noticeInfo","noticeinfo"}};
    ...
    // 数据表中所有日期类型的字段
    private static final String[] _dateFields = {"STARTDATE","ENDDATE"};
    ...
    public String getNoticeName() {  return SqlTool.getDbString(noticeName);  }
    public String getNoticeName(int size) { return SqlTool.subString(getNoticeName(), size); }
    public void setNoticeName(String aNoticeName) { noticeName = aNoticeName; modifiedFields.add("NoticeName"); refreshFlag = true; }
    public void assignNoticeName(String aNoticeName) { noticeName = aNoticeName; }
    public void paramNoticeName(HttpServletRequest request) { paramNoticeName(request, "NoticeName"); }
    public void paramNoticeName(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getNoticeName()); if (!tmp.equals(getNoticeName())) { setNoticeName(tmp); } }
    // 开始日期
    ...
    // 结束日期
    protected java.util.Date enddate = java.sql.Date.valueOf("0001-01-01");
    public java.util.Date getEnddate() { return enddate; }
    public void setEnddate(java.util.Date aEnddate) { enddate = aEnddate; modifiedFields.add("Enddate"); }
    public void assignEnddate(java.util.Date aEnddate) { enddate = aEnddate; }
    public void paramEnddate(HttpServletRequest request) { paramEnddate(request, "Enddate"); }
    public void paramEnddate(HttpServletRequest request, String name) { java.util.Date tmp = ParamUtils.getDateTimeParameter(request, name, getEnddate()); if (tmp.compareTo(getEnddate()) != 0) { setEnddate(tmp); } }
    // 通知内容
    protected String noticeInfo = "";
    public String getNoticeInfo() {  return SqlTool.getDbString(noticeInfo);  }
    public String getNoticeInfo(int size) { return SqlTool.subString(getNoticeInfo(), size); }
    public void setNoticeInfo(String aNoticeInfo) { noticeInfo = aNoticeInfo; modifiedFields.add("NoticeInfo"); }
    public void assignNoticeInfo(String aNoticeInfo) { noticeInfo = aNoticeInfo; }
    public void paramNoticeInfo(HttpServletRequest request) { paramNoticeInfo(request, "NoticeInfo"); }
    public void paramNoticeInfo(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getNoticeInfo()); if (!tmp.equals(getNoticeInfo())) { setNoticeInfo(tmp); } }
    ...
    public VolunteerZhiku (String db) {
        DBNAME = db;
    }
    public VolunteerZhiku (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public VolunteerZhiku (HttpServletRequest request) {
    ...
    public VolunteerZhiku () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return VolunteerZhiku类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public VolunteerZhiku getByParameter(HttpServletRequest request) {
        VolunteerZhiku v = new VolunteerZhiku(DBNAME, TNAME);
        v.assignParameter(request);
        return v;
    }
    public void assignParameter(HttpServletRequest request) {
        paramId(request);
        paramNoticeName(request);
        paramStartdate(request);
        paramEnddate(request);
        paramNoticeInfo(request);
    }
    /**
     * 将参数中类的实例数据复制到当前的类实例，noChangeList中包含属性名将不被复制。
    */
    public void clone(VolunteerZhiku bean) {
        List ncl = Arrays.asList(noChangeList);
        if (!ncl.contains("id")) { setId(bean.getId()); }
        if (!ncl.contains("noticeName")) { setNoticeName(bean.getNoticeName()); }
        if (!ncl.contains("startdate")) { setStartdate(bean.getStartdate()); }
        if (!ncl.contains("enddate")) { setEnddate(bean.getEnddate()); }
        if (!ncl.contains("noticeInfo")) { setNoticeInfo(bean.getNoticeInfo()); }
    }
    public VolunteerZhiku getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        VolunteerZhiku[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                VolunteerZhiku ve = new VolunteerZhiku(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignNoticeName(fromDbCh(rs.getString(2)));
                tmpDate = rs.getTimestamp(3); if (tmpDate != null) { ve.assignStartdate(new java.util.Date(tmpDate.getTime())); }
                tmpDate = rs.getTimestamp(4); if (tmpDate != null) { ve.assignEnddate(new java.util.Date(tmpDate.getTime())); }
                ve.assignNoticeInfo(fromDbCh(rs.getString(5)));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public VolunteerZhiku[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (VolunteerZhiku[])vrs.toArray(new VolunteerZhiku[vrs.size()]);
    }
    public VolunteerZhiku[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (noticename,startdate,enddate,noticeinfo) VALUES (?,?,?,?)";
        ...
        CEditConst.setRefreshFlag(DBNAME, "com.xietong.software.sdtw.db.VolunteerZhiku");
        clearModifiedFields();
        return iResult;
    }
    /**
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,noticename,startdate,enddate,noticeinfo) VALUES (?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            VolunteerZhiku item = (VolunteerZhiku)it.next();
            VolunteerZhiku v = new VolunteerZhiku(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                int pi = 1;
                if (modifiedFields.contains("NoticeName")) { pstmt.setString(pi ++, toDbCh(noticeName)); }
                if (modifiedFields.contains("Startdate")) { pstmt.setTimestamp(pi ++, new java.sql.Timestamp(startdate.getTime())); }
                if (modifiedFields.contains("Enddate")) { pstmt.setTimestamp(pi ++, new java.sql.Timestamp(enddate.getTime())); }
                if (modifiedFields.contains("NoticeInfo")) { pstmt.setString(pi ++, toDbCh(noticeInfo)); }
                pstmt.setInt(pi, id);
                iResult = pstmt.executeUpdate();
            }
            catch( SQLException sqle ) {
    ...
    */
    ...
    public Map toMap() {
        Map map = new HashMap();
        map.put("Id", getId() + "");
        map.put("NoticeName", getNoticeName());
        map.put("Startdate", Tool.stringOfDateTime(getStartdate()));
        map.put("Enddate", Tool.stringOfDateTime(getEnddate()));
        map.put("NoticeInfo", getNoticeInfo());
        return map;
    }
    public String toJsMap() {
        List row = new ArrayList();
        row.add("Id:\"" + getId() + "\"");
        row.add("NoticeName:\"" + Tool.jsSpecialChars(getNoticeName()) + "\"");
        row.add("Startdate:\"" + Tool.stringOfDateTime(getStartdate()) + "\"");
        row.add("Enddate:\"" + Tool.stringOfDateTime(getEnddate()) + "\"");
        row.add("NoticeInfo:\"" + Tool.jsSpecialChars(getNoticeInfo()) + "\"");
        return "{" + Tool.join(",", row) + "}";
    }
    public int init() {
        List sqlList= new ArrayList();
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,noticename varchar2(45),startdate date,enddate date,noticeinfo varchar2(2000))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        VolunteerZhiku v = new VolunteerZhiku(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        VolunteerZhiku v = new VolunteerZhiku(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(VolunteerZhiku obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = getNoticeName().compareTo(obj.getNoticeName()); if (rtn != 0) return rtn;
        rtn = getStartdate().compareTo(obj.getStartdate()); if (rtn != 0) return rtn;
        rtn = getEnddate().compareTo(obj.getEnddate()); if (rtn != 0) return rtn;
        rtn = getNoticeInfo().compareTo(obj.getNoticeInfo()); if (rtn != 0) return rtn;
        return rtn;
    }
}
