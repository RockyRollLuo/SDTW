...
@Description("民族")
public class Nations implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    private static Log log = LogFactory.getLog(Nations.class);
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "nations";
    private String TNAME = "nations";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","name"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","name"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Name","name","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"name","name"}};
    ...
    public Nations (String db) {
        DBNAME = db;
    }
    public Nations (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public Nations (HttpServletRequest request) {
     ...
     * @return Nations类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public Nations getByParameter(HttpServletRequest request) {
        Nations v = new Nations(DBNAME, TNAME);
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                Nations ve = new Nations(DBNAME, TNAME);
    ...
    public Nations[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (Nations[])vrs.toArray(new Nations[vrs.size()]);
    }
    public Nations[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (name) VALUES (?)";
        ...
        CEditConst.setRefreshFlag(DBNAME, "com.xietong.software.sdtw.db.Nations");
        clearModifiedFields();
        return iResult;
    }
    /**
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,name) VALUES (?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            Nations item = (Nations)it.next();
            Nations v = new Nations(dbname);
    ...
    */
    ...
    public int init() {
        ...
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0),name varchar2(45))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        Nations v = new Nations(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        Nations v = new Nations(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(Nations obj) {
        int rtn = 0;
