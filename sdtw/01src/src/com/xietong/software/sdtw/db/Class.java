...
@Description("年级")
public class Class implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "class";
    private String TNAME = "class";
    ...
    public Class (String db) {
        DBNAME = db;
    }
    public Class (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public Class (HttpServletRequest request) {
     ...
     * @return Class类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public Class getByParameter(HttpServletRequest request) {
        Class v = new Class(DBNAME, TNAME);
    ...
    */
    ...
    public Class getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        Class[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                Class ve = new Class(DBNAME, TNAME);
    ...
    public Class[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (Class[])vrs.toArray(new Class[vrs.size()]);
    }
    public Class[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        Class v = new Class(sourceDb, sourceTb);
