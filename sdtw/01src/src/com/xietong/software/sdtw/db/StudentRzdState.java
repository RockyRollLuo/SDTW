...
@Description("学生素质拓展培养认证单打印记录表")
public class StudentRzdState implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "student_rzd_state";
    private String TNAME = "student_rzd_state";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","stuid","printid","idlist","printcode","printTime"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","stuid","printid","idlist","printcode","printtime"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Stuid","stuid","int"},{"Printid","printid","int"},{"Idlist","idlist","String"},{"Printcode","printcode","String"},{"PrintTime","printtime","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"stuid","stuid"},{"printid","printid"},{"idlist","idlist"},{"printcode","printcode"},{"printTime","printtime"}};
    ...
    public int getPrintid() { return printid; }
    public void setPrintid(int aPrintid) { printid = aPrintid; modifiedFields.add("Printid"); }
    public void assignPrintid(int aPrintid) { printid = aPrintid; }
    public void paramPrintid(HttpServletRequest request) { paramPrintid(request, "Printid"); }
    public void paramPrintid(HttpServletRequest request, String name) { int tmp = ParamUtils.getIntParameter(request, name, getPrintid()); if (tmp != getPrintid()) { setPrintid(tmp); } }
    // 打印项
    protected String idlist = "";
    public String getIdlist() {  return SqlTool.getDbString(idlist);  }
    public String getIdlist(int size) { return SqlTool.subString(getIdlist(), size); }
    public void setIdlist(String aIdlist) { idlist = aIdlist; modifiedFields.add("Idlist"); }
    public void assignIdlist(String aIdlist) { idlist = aIdlist; }
    public void paramIdlist(HttpServletRequest request) { paramIdlist(request, "Idlist"); }
    public void paramIdlist(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getIdlist()); if (!tmp.equals(getIdlist())) { setIdlist(tmp); } }
    // 打印编码
    ...
    public String getPrintTime() {  return SqlTool.getDbString(printTime);  }
    public String getPrintTime(int size) { return SqlTool.subString(getPrintTime(), size); }
    public void setPrintTime(String aPrintTime) { printTime = aPrintTime; modifiedFields.add("PrintTime"); }
    public void assignPrintTime(String aPrintTime) { printTime = aPrintTime; }
    public void paramPrintTime(HttpServletRequest request) { paramPrintTime(request, "PrintTime"); }
    public void paramPrintTime(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getPrintTime()); if (!tmp.equals(getPrintTime())) { setPrintTime(tmp); } }
    ...
    public StudentRzdState (String db) {
        DBNAME = db;
    }
    public StudentRzdState (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public StudentRzdState (HttpServletRequest request) {
    ...
    public StudentRzdState () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return StudentRzdState类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public StudentRzdState getByParameter(HttpServletRequest request) {
        StudentRzdState v = new StudentRzdState(DBNAME, TNAME);
    ...
    public void assignParameter(HttpServletRequest request) {
        paramId(request);
        paramStuid(request);
        paramPrintid(request);
        paramIdlist(request);
        paramPrintcode(request);
        paramPrintTime(request);
    }
    /**
     * 将参数中类的实例数据复制到当前的类实例，noChangeList中包含属性名将不被复制。
    */
    public void clone(StudentRzdState bean) {
        List ncl = Arrays.asList(noChangeList);
        if (!ncl.contains("id")) { setId(bean.getId()); }
        if (!ncl.contains("stuid")) { setStuid(bean.getStuid()); }
        if (!ncl.contains("printid")) { setPrintid(bean.getPrintid()); }
        if (!ncl.contains("idlist")) { setIdlist(bean.getIdlist()); }
        if (!ncl.contains("printcode")) { setPrintcode(bean.getPrintcode()); }
        if (!ncl.contains("printTime")) { setPrintTime(bean.getPrintTime()); }
    }
    public StudentRzdState getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        StudentRzdState[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                StudentRzdState ve = new StudentRzdState(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignStuid(rs.getInt(2));
                ve.assignPrintid(rs.getInt(3));
                ve.assignIdlist(fromDbCh(rs.getString(4)));
                ve.assignPrintcode(fromDbCh(rs.getString(5)));
                ve.assignPrintTime(fromDbCh(rs.getString(6)));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public StudentRzdState[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (StudentRzdState[])vrs.toArray(new StudentRzdState[vrs.size()]);
    }
    public StudentRzdState[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (stuid,printid,idlist,printcode,printtime) VALUES (?,?,?,?,?)";
        ...
        try {
            ...
            pstmt.setInt(2, printid);
            pstmt.setString(3, toDbCh(idlist));
            pstmt.setString(4, toDbCh(printcode));
            pstmt.setString(5, toDbCh(printTime));
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,stuid,printid,idlist,printcode,printtime) VALUES (?,?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            pstmt.setInt(3, printid);
            pstmt.setString(4, toDbCh(idlist));
            pstmt.setString(5, toDbCh(printcode));
            pstmt.setString(6, toDbCh(printTime));
            iResult = pstmt.executeUpdate();
        }
        catch( SQLException sqle ) {
            sqle.printStackTrace();
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            StudentRzdState item = (StudentRzdState)it.next();
            StudentRzdState v = new StudentRzdState(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                if (modifiedFields.contains("Printid")) { pstmt.setInt(pi ++, printid); }
                if (modifiedFields.contains("Idlist")) { pstmt.setString(pi ++, toDbCh(idlist)); }
                if (modifiedFields.contains("Printcode")) { pstmt.setString(pi ++, toDbCh(printcode)); }
                if (modifiedFields.contains("PrintTime")) { pstmt.setString(pi ++, toDbCh(printTime)); }
                pstmt.setInt(pi, id);
                iResult = pstmt.executeUpdate();
            }
            catch( SQLException sqle ) {
    ...
    */
    ...
    public Map toMap() {
        ...
        map.put("Printid", getPrintid() + "");
        map.put("Idlist", getIdlist());
    ...
    public String toJsMap() {
        ...
        row.add("Printid:\"" + getPrintid() + "\"");
        row.add("Idlist:\"" + Tool.jsSpecialChars(getIdlist()) + "\"");
        row.add("Printcode:\"" + Tool.jsSpecialChars(getPrintcode()) + "\"");
        row.add("PrintTime:\"" + Tool.jsSpecialChars(getPrintTime()) + "\"");
        return "{" + Tool.join(",", row) + "}";
    }
    public int init() {
        List sqlList= new ArrayList();
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,stuid number(*,0),printid number(*,0),idlist varchar2(2000),printcode varchar2(45),printtime varchar2(45))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        StudentRzdState v = new StudentRzdState(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        StudentRzdState v = new StudentRzdState(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(StudentRzdState obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = ("" + getStuid()).compareTo("" + obj.getStuid()); if (rtn != 0) return rtn;
        rtn = ("" + getPrintid()).compareTo("" + obj.getPrintid()); if (rtn != 0) return rtn;
        rtn = getIdlist().compareTo(obj.getIdlist()); if (rtn != 0) return rtn;
        rtn = getPrintcode().compareTo(obj.getPrintcode()); if (rtn != 0) return rtn;
        rtn = getPrintTime().compareTo(obj.getPrintTime()); if (rtn != 0) return rtn;
        return rtn;
    }
}
