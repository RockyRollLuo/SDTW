...
public class ClubModifyTeacher implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "club_modify_teacher";
    private String TNAME = "club_modify_teacher";
    ...
    public ClubModifyTeacher (String db) {
        DBNAME = db;
    }
    public ClubModifyTeacher (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public ClubModifyTeacher (HttpServletRequest request) {
    ...
    public ClubModifyTeacher () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return ClubModifyTeacher类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public ClubModifyTeacher getByParameter(HttpServletRequest request) {
        ClubModifyTeacher v = new ClubModifyTeacher(DBNAME, TNAME);
    ...
    */
    public void clone(ClubModifyTeacher bean) {
    ...
    public ClubModifyTeacher getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        ClubModifyTeacher[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
     ...
     * @return ClubModifyTeacher类的实例列表。
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                ClubModifyTeacher ve = new ClubModifyTeacher(DBNAME, TNAME);
    ...
    public ClubModifyTeacher[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (ClubModifyTeacher[])vrs.toArray(new ClubModifyTeacher[vrs.size()]);
    }
    public ClubModifyTeacher[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            ClubModifyTeacher item = (ClubModifyTeacher)it.next();
            ClubModifyTeacher v = new ClubModifyTeacher(dbname);
    ...
    */
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        ClubModifyTeacher v = new ClubModifyTeacher(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        ClubModifyTeacher v = new ClubModifyTeacher(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(ClubModifyTeacher obj) {
        int rtn = 0;
