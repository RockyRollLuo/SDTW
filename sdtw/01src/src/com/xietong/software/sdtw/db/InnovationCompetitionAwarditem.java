...
@Description("创业竞赛比赛奖励表")
public class InnovationCompetitionAwarditem implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "innovation_competition_awarditem";
    private String TNAME = "innovation_competition_awarditem";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","itemname","addtime","addperson","moditime","modiperson"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","itemname","addtime","addperson","moditime","modiperson"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Itemname","itemname","String"},{"Addtime","addtime","java.util.Date"},{"Addperson","addperson","String"},{"Moditime","moditime","java.util.Date"},{"Modiperson","modiperson","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"itemname","itemname"},{"addtime","addtime"},{"addperson","addperson"},{"moditime","moditime"},{"modiperson","modiperson"}};
    ...
    public String getItemname() {  return SqlTool.getDbString(itemname);  }
    public String getItemname(int size) { return SqlTool.subString(getItemname(), size); }
    public void setItemname(String aItemname) { itemname = aItemname; modifiedFields.add("Itemname"); }
    public void assignItemname(String aItemname) { itemname = aItemname; }
    public void paramItemname(HttpServletRequest request) { paramItemname(request, "Itemname"); }
    public void paramItemname(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getItemname()); if (!tmp.equals(getItemname())) { setItemname(tmp); } }
    // 添加时间
    ...
    public InnovationCompetitionAwarditem (String db) {
        DBNAME = db;
    }
    public InnovationCompetitionAwarditem (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public InnovationCompetitionAwarditem (HttpServletRequest request) {
    ...
    public InnovationCompetitionAwarditem () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return InnovationCompetitionAwarditem类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public InnovationCompetitionAwarditem getByParameter(HttpServletRequest request) {
        InnovationCompetitionAwarditem v = new InnovationCompetitionAwarditem(DBNAME, TNAME);
        v.assignParameter(request);
        return v;
    }
    public void assignParameter(HttpServletRequest request) {
        paramId(request);
        paramItemname(request);
    ...
    */
    public void clone(InnovationCompetitionAwarditem bean) {
        List ncl = Arrays.asList(noChangeList);
        if (!ncl.contains("id")) { setId(bean.getId()); }
        if (!ncl.contains("itemname")) { setItemname(bean.getItemname()); }
    ...
    public InnovationCompetitionAwarditem getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        InnovationCompetitionAwarditem[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
     ...
     * @return InnovationCompetitionAwarditem类的实例列表。
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                InnovationCompetitionAwarditem ve = new InnovationCompetitionAwarditem(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignItemname(fromDbCh(rs.getString(2)));
                tmpDate = rs.getTimestamp(3); if (tmpDate != null) { ve.assignAddtime(new java.util.Date(tmpDate.getTime())); }
                ve.assignAddperson(fromDbCh(rs.getString(4)));
                tmpDate = rs.getTimestamp(5); if (tmpDate != null) { ve.assignModitime(new java.util.Date(tmpDate.getTime())); }
                ve.assignModiperson(fromDbCh(rs.getString(6)));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public InnovationCompetitionAwarditem[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (InnovationCompetitionAwarditem[])vrs.toArray(new InnovationCompetitionAwarditem[vrs.size()]);
    }
    public InnovationCompetitionAwarditem[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (itemname,addtime,addperson,moditime,modiperson) VALUES (?,?,?,?,?)";
        ...
        try {
            ...
            pstmt.setString(1, toDbCh(itemname));
            pstmt.setTimestamp(2, new java.sql.Timestamp(addtime.getTime()));
            pstmt.setString(3, toDbCh(addperson));
            pstmt.setTimestamp(4, new java.sql.Timestamp(moditime.getTime()));
            pstmt.setString(5, toDbCh(modiperson));
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,itemname,addtime,addperson,moditime,modiperson) VALUES (?,?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            stmt = con.createStatement();
            pstmt.setInt(1, id);
            pstmt.setString(2, toDbCh(itemname));
            pstmt.setTimestamp(3, new java.sql.Timestamp(addtime.getTime()));
            pstmt.setString(4, toDbCh(addperson));
            pstmt.setTimestamp(5, new java.sql.Timestamp(moditime.getTime()));
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            InnovationCompetitionAwarditem item = (InnovationCompetitionAwarditem)it.next();
            InnovationCompetitionAwarditem v = new InnovationCompetitionAwarditem(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                int pi = 1;
                if (modifiedFields.contains("Itemname")) { pstmt.setString(pi ++, toDbCh(itemname)); }
    ...
    */
    ...
    public String toJsMap() {
        ...
        row.add("Id:\"" + getId() + "\"");
        row.add("Itemname:\"" + Tool.jsSpecialChars(getItemname()) + "\"");
    ...
    public int init() {
        ...
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0),itemname varchar2(45),addtime date,addperson varchar2(45),moditime date,modiperson varchar2(45))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        InnovationCompetitionAwarditem v = new InnovationCompetitionAwarditem(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        InnovationCompetitionAwarditem v = new InnovationCompetitionAwarditem(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(InnovationCompetitionAwarditem obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = getItemname().compareTo(obj.getItemname()); if (rtn != 0) return rtn;
