...
@Description("山大志库申请表团队成员表")
public class VolunteerZhikuTeammember implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "volunteer_zhiku_teammember";
    private String TNAME = "volunteer_zhiku_teammember";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","zhikuDeclareId","name","sex","academy","grade","sno","phone","duty"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","zhikudeclareid","name","sex","academy","grade","sno","phone","duty"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"ZhikuDeclareId","zhikudeclareid","int"},{"Name","name","String"},{"Sex","sex","String"},{"Academy","academy","String"},{"Grade","grade","String"},{"Sno","sno","String"},{"Phone","phone","String"},{"Duty","duty","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"zhikuDeclareId","zhikudeclareid"},{"name","name"},{"sex","sex"},{"academy","academy"},{"grade","grade"},{"sno","sno"},{"phone","phone"},{"duty","duty"}};
    ...
    // 志库申请表id
    protected int zhikuDeclareId = -1;
    public int getZhikuDeclareId() { return zhikuDeclareId; }
    public void setZhikuDeclareId(int aZhikuDeclareId) { zhikuDeclareId = aZhikuDeclareId; modifiedFields.add("ZhikuDeclareId"); }
    public void assignZhikuDeclareId(int aZhikuDeclareId) { zhikuDeclareId = aZhikuDeclareId; }
    public void paramZhikuDeclareId(HttpServletRequest request) { paramZhikuDeclareId(request, "ZhikuDeclareId"); }
    public void paramZhikuDeclareId(HttpServletRequest request, String name) { int tmp = ParamUtils.getIntParameter(request, name, getZhikuDeclareId()); if (tmp != getZhikuDeclareId()) { setZhikuDeclareId(tmp); } }
    // 姓名
    ...
    public VolunteerZhikuTeammember (String db) {
        DBNAME = db;
    }
    public VolunteerZhikuTeammember (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public VolunteerZhikuTeammember (HttpServletRequest request) {
    ...
    public VolunteerZhikuTeammember () {
    }
    /**
     * 提交的form中的输入域名符合bean属性名，将被赋值给对应的bean属性。
     * @param request包含从http请求提交过来的form信息
     * @return VolunteerZhikuTeammember类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public VolunteerZhikuTeammember getByParameter(HttpServletRequest request) {
        VolunteerZhikuTeammember v = new VolunteerZhikuTeammember(DBNAME, TNAME);
        v.assignParameter(request);
        return v;
    }
    public void assignParameter(HttpServletRequest request) {
        paramId(request);
        paramZhikuDeclareId(request);
        paramName(request);
        paramSex(request);
    ...
    */
    public void clone(VolunteerZhikuTeammember bean) {
        List ncl = Arrays.asList(noChangeList);
        if (!ncl.contains("id")) { setId(bean.getId()); }
        if (!ncl.contains("zhikuDeclareId")) { setZhikuDeclareId(bean.getZhikuDeclareId()); }
    ...
    public VolunteerZhikuTeammember getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        VolunteerZhikuTeammember[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
     ...
     * @return VolunteerZhikuTeammember类的实例列表。
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                VolunteerZhikuTeammember ve = new VolunteerZhikuTeammember(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignZhikuDeclareId(rs.getInt(2));
                ve.assignName(fromDbCh(rs.getString(3)));
                ve.assignSex(fromDbCh(rs.getString(4)));
                ve.assignAcademy(fromDbCh(rs.getString(5)));
                ve.assignGrade(fromDbCh(rs.getString(6)));
                ve.assignSno(fromDbCh(rs.getString(7)));
    ...
    public VolunteerZhikuTeammember[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (VolunteerZhikuTeammember[])vrs.toArray(new VolunteerZhikuTeammember[vrs.size()]);
    }
    public VolunteerZhikuTeammember[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (zhikudeclareid,name,sex,academy,grade,sno,phone,duty) VALUES (?,?,?,?,?,?,?,?)";
        ...
        try {
            ...
            pstmt.setInt(1, zhikuDeclareId);
            pstmt.setString(2, toDbCh(name));
            pstmt.setString(3, toDbCh(sex));
            pstmt.setString(4, toDbCh(academy));
            pstmt.setString(5, toDbCh(grade));
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,zhikudeclareid,name,sex,academy,grade,sno,phone,duty) VALUES (?,?,?,?,?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            VolunteerZhikuTeammember item = (VolunteerZhikuTeammember)it.next();
            VolunteerZhikuTeammember v = new VolunteerZhikuTeammember(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                int pi = 1;
                if (modifiedFields.contains("ZhikuDeclareId")) { pstmt.setInt(pi ++, zhikuDeclareId); }
    ...
    */
    ...
    public Map toMap() {
        Map map = new HashMap();
        map.put("Id", getId() + "");
        map.put("ZhikuDeclareId", getZhikuDeclareId() + "");
    ...
    public String toJsMap() {
        ...
        row.add("Id:\"" + getId() + "\"");
        row.add("ZhikuDeclareId:\"" + getZhikuDeclareId() + "\"");
    ...
    public int init() {
        ...
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,zhikudeclareid number(*,0),name varchar2(45),sex varchar2(45),academy varchar2(45),grade varchar2(45),sno varchar2(45),phone varchar2(45),duty varchar2(45))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        VolunteerZhikuTeammember v = new VolunteerZhikuTeammember(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        VolunteerZhikuTeammember v = new VolunteerZhikuTeammember(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(VolunteerZhikuTeammember obj) {
        int rtn = 0;
        rtn = ("" + getId()).compareTo("" + obj.getId()); if (rtn != 0) return rtn;
        rtn = ("" + getZhikuDeclareId()).compareTo("" + obj.getZhikuDeclareId()); if (rtn != 0) return rtn;
