...
@Description("单位信息")
public class BaseOrgCode implements DaoBase {
    /**
     * 所有私有变量
    */
    // 日志
    private static Log log = LogFactory.getLog(BaseOrgCode.class);
    ...
    // 保存当前连接的数据表名
    public static final String DbTableName = "base_orgcode";
    private String TNAME = "base_orgcode";
    ...
    // 包含所有bean属性名数组
    private static final String[] _allProperties = {"id","name","code","orgAddress","orderNum","isEnable","opLog"};
    public String[] getAllProperties() { return _allProperties; }
    private static final String[] _allFields = {"id","name","code","orgaddress","ordernum","isenable","oplog"};
    public String[] getAllFields() {return _allFields;}
    private static final String[][] _arrayPF = {{"Id","id","int"},{"Name","name","String"},{"Code","code","String"},{"OrgAddress","orgaddress","String"},{"OrderNum","ordernum","int"},{"IsEnable","isenable","int"},{"OpLog","oplog","String"}};
    private static final String[][] _allPropertiesAndFields = {{"id","id"},{"name","name"},{"code","code"},{"orgAddress","orgaddress"},{"orderNum","ordernum"},{"isEnable","isenable"},{"opLog","oplog"}};
    ...
    public String getOrgAddress() {  return SqlTool.getDbString(orgAddress);  }
    public String getOrgAddress(int size) { return SqlTool.subString(getOrgAddress(), size); }
    public void setOrgAddress(String aOrgAddress) { orgAddress = aOrgAddress; modifiedFields.add("OrgAddress"); }
    public void assignOrgAddress(String aOrgAddress) { orgAddress = aOrgAddress; }
    public void paramOrgAddress(HttpServletRequest request) { paramOrgAddress(request, "OrgAddress"); }
    public void paramOrgAddress(HttpServletRequest request, String name) { String tmp = ParamUtils.getParameter(request, name, getOrgAddress()); if (!tmp.equals(getOrgAddress())) { setOrgAddress(tmp); } }
    // 排序
    ...
    public BaseOrgCode (String db) {
        DBNAME = db;
    }
    public BaseOrgCode (String db, String tname) {
        if (db != null) {
            DBNAME = db;
        }
        if (tname != null) {
    ...
    public BaseOrgCode (HttpServletRequest request) {
     ...
     * @return BaseOrgCode类的实例，包含了从http请求获得信息放到各个对应属性中
    */
    public BaseOrgCode getByParameter(HttpServletRequest request) {
        BaseOrgCode v = new BaseOrgCode(DBNAME, TNAME);
    ...
    */
    public void clone(BaseOrgCode bean) {
        ...
        if (!ncl.contains("orgAddress")) { setOrgAddress(bean.getOrgAddress()); }
    ...
    public BaseOrgCode getById(int aId) {
        List cdt = new ArrayList();
        cdt.add("id=" + aId);
        BaseOrgCode[] rslt= queryByCondition(cdt);
        if (rslt.length == 1) {
            return rslt[0];
        }
        else {
    ...
    */
    ...
    public List query(Connection con, List cdt) {
        ...
        try {
            ...
            java.sql.Timestamp tmpDate = null;
            while (rs.next()) {
                BaseOrgCode ve = new BaseOrgCode(DBNAME, TNAME);
                ve.assignId(rs.getInt(1));
                ve.assignName(fromDbCh(rs.getString(2)));
                ve.assignCode(fromDbCh(rs.getString(3)));
                ve.assignOrgAddress(fromDbCh(rs.getString(4)));
                ve.assignOrderNum(rs.getInt(5));
                ve.assignIsEnable(rs.getInt(6));
                ve.assignOpLog(fromDbCh(rs.getString(7)));
                vrs.add(ve);
            }
        }
        catch( SQLException sqle ) {
    ...
    public BaseOrgCode[] queryByCondition(List cdt) {
        List vrs = query(cdt);
        return (BaseOrgCode[])vrs.toArray(new BaseOrgCode[vrs.size()]);
    }
    public BaseOrgCode[] queryByCondition(Connection con, List cdt) {
    ...
    */
    ...
    public int insert(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (name,code,orgaddress,ordernum,isenable,oplog) VALUES (?,?,?,?,?,?)";
        ...
        try {
            ...
            pstmt.setString(3, toDbCh(orgAddress));
            pstmt.setInt(4, orderNum);
            pstmt.setInt(5, isEnable);
            pstmt.setString(6, toDbCh(opLog));
        ...
        CEditConst.setRefreshFlag(DBNAME, "com.xietong.software.sdtw.db.BaseOrgCode");
        clearModifiedFields();
        return iResult;
    }
    /**
    ...
    */
    ...
    public int insertByKey(Connection con) {
        String INSERT = "INSERT INTO " + getDBNAME() + " (id,name,code,orgaddress,ordernum,isenable,oplog) VALUES (?,?,?,?,?,?,?)";
        int iResult = -1;
        PreparedStatement pstmt = null;
        Statement stmt = null;
        ResultSet rs = null;
        try {
            ...
            pstmt.setString(4, toDbCh(orgAddress));
            pstmt.setInt(5, orderNum);
            pstmt.setInt(6, isEnable);
            pstmt.setString(7, toDbCh(opLog));
            iResult = pstmt.executeUpdate();
        }
        catch( SQLException sqle ) {
            sqle.printStackTrace();
    ...
    */
    ...
    public boolean batchInsertByKey(List list, String dbname) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            ...
            BaseOrgCode v = new BaseOrgCode(dbname);
    ...
    public int update(Connection con) {
        ...
        if (updateFields.size() > 0) {
            ...
            try {
                ...
                if (modifiedFields.contains("OrgAddress")) { pstmt.setString(pi ++, toDbCh(orgAddress)); }
    ...
    */
    ...
    public TreeItem cloneTi(BaseOrgCode item) {
    ...
    public String findRootKey(Map parent) {
        ...
        for (Iterator mit = parent.keySet().iterator(); mit.hasNext();) {
            ...
            for (Iterator it = list.iterator(); it.hasNext();) {
                BaseOrgCode v = (BaseOrgCode)it.next();
                allids.add("" + v.getId());
            }
        }
        for (Iterator mit = parent.keySet().iterator(); mit.hasNext();) {
    ...
    public TreeItem initTree(BaseOrgCode item, Map parent) {
    ...
    public Map toMap() {
        ...
        map.put("OrgAddress", getOrgAddress());
    ...
    public String toJsMap() {
        ...
        row.add("OrgAddress:\"" + Tool.jsSpecialChars(getOrgAddress()) + "\"");
    ...
    public int init() {
        ...
        sqlList.add(new String[]{"create", "create table :DBNAME:.:TNAME: (id number(*,0) not null,name varchar2(200) not null,code varchar2(200),orgaddress varchar2(1000),ordernum number(*,0),isenable number(*,0),oplog varchar2(1000))"});
    ...
    public void duplicate(String sourceDb, String sourceTb, List cdt) {
        BaseOrgCode v = new BaseOrgCode(sourceDb, sourceTb);
    ...
    public void duplicate(String sourceDb, List cdt) {
        BaseOrgCode v = new BaseOrgCode(sourceDb);
        int num = v.getCount(cdt);
        for (int i = 0; i < num; i += 100) {
            cdt.clear();
            cdt.add("order by id asc");
    ...
    public int compareTo(BaseOrgCode obj) {
        int rtn = 0;
        ...
        rtn = getOrgAddress().compareTo(obj.getOrgAddress()); if (rtn != 0) return rtn;
