			   ...
			   if (chkeds.length > 0) {
					...
					for(var i =0;i<rowIndex.length;i++){
						...
						if(values==""){
							...
							reName += "<tr id='deleteSponsor_"+reIdtmp+"'><td>"+rows[rowIndex[i]][getCodeIndex('Name')]+"</td><td>"+rows[rowIndex[i]][getCodeIndex('CnName')]+"</td><td>"+rows[rowIndex[i]][getCodeIndex('DeptId')]+"</td><td><input size='10' name='phone"+i+"'/></td><td><input size='10' name='zhiwu"+i+"' id='zhiwu"+i+"'/>"+"</td><td><img style=\"cursor:point;\" src='<%=HeadConst.apache_url%>/main/images/delete.gif' onclick=javascript:deleteSponsor(\'deleteSponsor_"+reIdtmp+"\',\'"+reIdtmp+"\') alt='删除' title='删除' border=0 /></td></tr>";
						}
						else{ 
							if((","+values+",").indexOf(","+reIdtmp+",")==-1){
								reId += ","+rows[rowIndex[i]][getCodeIndex('Id')];
								reName += "<tr id='deleteSponsor_"+reIdtmp+"'><td>"+rows[rowIndex[i]][getCodeIndex('Name')]+"</td><td>"+rows[rowIndex[i]][getCodeIndex('CnName')]+"<td>"+rows[rowIndex[i]][getCodeIndex('DeptId')]+"</td><td><input size='10' name='phone"+i+"'/></td><td><input size='10' name='zhiwu"+i+"' id='zhiwu"+i+"'/>"+"</td><td><img style=\"cursor:point;\" src='<%=HeadConst.apache_url%>/main/images/delete.gif' onclick=javascript:deleteSponsor(\'deleteSponsor_"+reIdtmp+"\',\'"+reIdtmp+"\') alt='删除' title='删除' border=0 /></td></tr>";
								slist=slist+1;
							}else{
								reId = values;
							}
