...
//private String[] ListFields = {"OrgId", "DeptId", "Name", "CnName", "Code", "Gender", "Occupation", "PhoneNum", "MobilePhone", "Email", "Roles", "MemberPolitics", "Birthday", "ZhiBu", "Nation", "Major", "LenOfSchool", "NativePlace", "IDNum", "Phone", "Email2", "Addfilelist", "YearToSchool", "GraduateTime", "VolunteerfFag"};
private String[] ListFields = {"OrgId", "DeptId", "Name", "CnName", "Gender","Birthday"};
...
private List getListRows(javax.servlet.http.HttpServletRequest request, BaseUserCode pv, List cdt)
{
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
         ...
         if(v.getDeptId()!=-1){
         row.add((String)AcademyNameMap.get("" + v.getDeptId()));}else{
        	  row.add("") ;
         }
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, BaseUserCode pv, boolean isAll)
{
    ...
    if(cmd.equals("teacher")){
    	 cdt.add("Roles !="+userInfo.getUserCode().getRoles());
    	 cdt.add("roles!=4");//不等于社团用户
    }else{
    	cdt.add("Roles="+userInfo.getUserCode().getRoles());
    }
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("list"))
    {
     ...
     if (cmd.equals("stuSponsor"))
    {
        setList(request);
        return mkRtn("stuSponsor", "success", form);
    }
     if (cmd.equals("teacher"))
     {
         setList(request);
         return mkRtn("teacher", "success", form);
     }
...
else if("stuMsg".equals(tmp)){
	int Id = ParamUtils.getIntParameter(request,"Id",-1);
	BaseUserCode user = new BaseUserCode();
	if(Id!=-1&&user.getById(Id)!=null){
		user = user.getById(Id);
		out.print("{\"msg\":\"ok\",\"MemberSno\":\""+user.getName()+"\",\"MemberName\":\""+user.getCnName()+"\",\"MemberSex\":\""+user.getGender()+"\",\"MemberBrith\":\""+Tool.stringOfDate(user.getBirthday())+"\",\"MemberPolitics\":\""+user.getMemberPolitics()+"\",\"MemberBirthplace\":\""+user.getNativePlace()+"\",\"MemberAcademy\":\""+user.getDeptId()+"\",\"MemberMajor\":\""+user.getMajor()+"\",\"MemberGrade\":\""+user.getGrade()+"\",\"MemberPhone\":\""+user.getPhone()+"\",\"Memberqq\":\""+user.getEmail2()+"\",\"MemberEmail\":\""+user.getEmail()+"\",\"Memberwx\":\""+user.getMobilePhone()+"\"}");
		//out.print("{\"msg\":\"ok\"}");
	}else{
		out.print("{\"msg\":\"no\"}");
	}
	return;
}else if("teaMsg".equals(tmp)){
	int Id = ParamUtils.getIntParameter(request,"Id",-1);
	BaseUserCode user = new BaseUserCode();
	if(Id!=-1&&user.getById(Id)!=null){
		user = user.getById(Id);
		//reId,ClubName,TeaName,TeaSex,TeaAge,TeaNation,TeaPolitics,TeaBirthplace,TeaDuty,TeaAcademy,TeaTel,TeaPhone,TeaEmail
		int year = Tool.yearOfDate();
		int birth = Tool.yearOfDate(user.getBirthday());
		System.out.print("{\"msg\":\"ok\",\"TeaName\":\""+user.getCnName()+"\",\"TeaSex\":\""+user.getGender()+"\",\"TeaAge\":\""+(year-birth)+"\",\"TeaNation\":\""+user.getNation()+"\",\"TeaPolitics\":\""+user.getMemberPolitics()+"\",\"TeaBirthplace\":\""+user.getNativePlace()+"\",\"TeaDuty\":\""+user.getOccupation()+"\",\"TeaAcademy\":\""+user.getDeptId()+"\",\"TeaTel\":\""+user.getPhoneNum()+"\",\"TeaPhone\":\""+user.getPhone()+"\",\"TeaEmail\":\""+user.getEmail()+"\"}");
		out.print("{\"msg\":\"ok\",\"TeaName\":\""+user.getCnName()+"\",\"TeaSex\":\""+user.getGender()+"\",\"TeaAge\":\""+(year-birth)+"\",\"TeaNation\":\""+user.getNation()+"\",\"TeaPolitics\":\""+user.getMemberPolitics()+"\",\"TeaBirthplace\":\""+user.getNativePlace()+"\",\"TeaDuty\":\""+user.getOccupation()+"\",\"TeaAcademy\":\""+user.getDeptId()+"\",\"TeaTel\":\""+user.getPhoneNum()+"\",\"TeaPhone\":\""+user.getPhone()+"\",\"TeaEmail\":\""+user.getEmail()+"\"}");
	}else{
		out.print("{\"msg\":\"no\"}");
	}
	return;
...
forwardMap.put("list", "BaseUserCodeAction.jsp");
forwardMap.put("failure", "BaseUserCodeForm.jsp");
forwardMap.put("success", "BaseUserCodeList.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script>window.location='BaseUserCodeAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
}
else {
