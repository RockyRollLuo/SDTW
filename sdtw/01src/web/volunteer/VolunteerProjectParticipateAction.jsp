...
private  VolunteerProjectParticipate getByParameterDb(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("update")) {
        ...
        v.paramMeanings(request, "Manings");
    }
    else {
    	v.setMeanings(ParamUtils.getParameter(request, "Manings",""));
        v.setId(ParamUtils.getIntParameter(request, "Id", -1));
        ...
        v.setXycheckFalg(ParamUtils.getIntParameter(request, "XycheckFalg", 0));
...
private List getListRows(javax.servlet.http.HttpServletRequest request, VolunteerProjectParticipate pv, List cdt)
{
	...
	private synchronized void  formInsert (VolunteerProjectParticipate pv){
	pv.insert();
}
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    {
        setForm(request, form);
        return mkRtn("save", "input1", form);
    }
    if (cmd.equals("modify"))
    {
    	 form = form.getById(form.getId());
         form.setSubmitFlag(1);
         setForm(request, form);
         form.update();
         return mkRtn("list", "input1", form);
    }
    if (cmd.equals("modify1"))
    {
        form = form.getById(form.getId());
        form.setSubmitFlag(0);
        setForm(request, form);
        form.update();
        return mkRtn("list", "input1", form);
    }
    if (cmd.equals("modify2"))
    {
    	int ppid=ParamUtils.getIntParameter(request, "Id", -1);
    	String sids=ParamUtils.getParameter(request, "SId", "-1");
    	
   	 String  [] ssids=Tool.split(",", sids);
   	// String zhiwu="zhiwu";
   	 String gongzuo="";
   	 List cdtmove=new ArrayList();
   	 System.out.println(ppid+"bbbbbbbbbbb"+sids);
   	cdtmove.add("projectparticipateid="+ppid);
   	VolunteerProjectTeammember vpm= new VolunteerProjectTeammember();
   	vpm.deleteByCondition(cdtmove);
    String dianhua="";
   	for(int i=0;i<ssids.length;i++){
   		vpm.setProjectParticipateId(ppid);
        ...
        System.out.println("AAAAAAAAAAAA"+form.getId());
        
       // setForm(request, form);
        System.out.println("BBBBB"+form.getProjectName());
      form.update();
	 ...
	 if(fl==-1){//上传失败
		...
		try{
			...
			if(attach.insert()>0){
				...
				System.out.println("114444444444441进入文件上传");
   		}else{
   			out.clear();
   			out.print("{error:'上传失败',msg:''}");
   		}
...
forwardMap.put("input1", "volunteer-project-list.jsp");
