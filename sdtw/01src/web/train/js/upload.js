	function uploadOtherAttatch(AttachType,FromClass,type){//上传其他附件
				var name=document.getElementById('OtherAttach'+type).value;
			    $("#load1"+type)
			    .ajaxStart(function(){
			        $(this).show();
			    })
			    ...
			    (
			        {
			            ...
			            secureuri:false,
			            fileElementId:'OtherAttach'+type,
			            type:'post',	
			            dataType: 'json',
			                    ...
			                    }else
			                    {
			                    	 if($('#'+type).val()==""){
			                            ...
			                            var be = data.url.indexOf("=");
			                         	var en = data.url.lastIndexOf("/");
			                         	//document.postForm.AddFileList.value += ","+data.ids;
			                         	$('#'+type).val($('#'+type).val()+","+data.ids);
			                         } 
			                        var table = document.getElementById('ShowFiles'+type);
			                        ...
			                        cell.innerHTML =  '<img src=\'../main/images/file1.gif\'/>'+filename+'&nbsp;&nbsp;&nbsp;&nbsp;<img src=\'../main/images/delete.gif\' alt=\'删除\' title=\'删除\' onclick=\'deleteUpdateFile("'+filename+'",'+data.ids+',"'+type+'");\' style="cursor:point;"/>';
			...
			function checkFileType(AttachType,FromClass,type){//上传文档格式过滤
				var file=document.getElementById('OtherAttach'+type).value;
				if(file){
					...
					if((fileExt+"").toLowerCase() in allowType){
						uploadOtherAttatch(AttachType,FromClass,type);
					}
					else{
			...
			function deleteUpdateFile(name,id,type){//删除修改时上传附件
				if(confirm('是否确定删除？')){
					$.ajax({
						type:"post",			
