...
private String[] ListFields = { "Name", "CnName", "Gender","Roles","OrgId", "DeptId"};
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, BaseUserCode pv, boolean isAll)
{
    ...
    QueryValues.add(qval);
    if (qval.trim().length() > 0&& !qval.equals("")) {
        cdt.add("deptid=" + qval);
    }
    
    qval = ParamUtils.getParameter(request, "_RoleName_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0&& !qval.equals("")) {
        cdt.add("roles=" + qval);
    }
