		...
		<script>
			   ...
			   if (chkeds.length > 0) {
					...
					for(var i =0;i<rowIndex.length;i++){
						var reIdtmp = rows[rowIndex[i]][getCodeIndex('Id')]+"";
							...
							reName += ","+rows[rowIndex[i]][getCodeIndex('CnName')];
			...
			function getCodeIndex(colu){
			 ...
			 function showList1(className, currpage, url_para, aHasCheckbox, cmd, haveSelect,curId) {
			    ...
			    if (hasCheckbox) {
			        str = str + "<td bgcolor=\"#809091\"><div class=\"browsetable004\"><input type=\"checkbox\" onclick=\"selectAll(this);\" onchange=\"selectAll(this);\"></div></td>\n";
			    }
			    for (var i = 0; i < fields.length; i ++) {
			        var title = dic[fields[i]][1];
			        title = title.replace(/\(.*\)/, "");
			        if (title == "") title = "&nbsp;";
			        str = str + "<td bgcolor=\"#809091\" onclick=\"javascript:setOrderBy('" + fields[i] + "', '" + url_para + "')\" nowrap><div class=\"browsetable004\">" + title;
