...
private String[] ListFields = {"PracticeName", "Starttime"};
...
private List getListRows(javax.servlet.http.HttpServletRequest request, Practice pv, List cdt)
{
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        row.add(Tool.jsSpecialChars(v.getStarttime()+"~"+v.getEndtime()));
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("save"))
    {
        ...
        list.add("山东大学"+Tool.yearOfDate()+"年度社会实践活动先进个人");
        list.add("山东大学"+Tool.yearOfDate()+"年度优秀社会实践报告（一等奖）");
        list.add("山东大学"+Tool.yearOfDate()+"年度优秀社会实践报告（二等奖）");
        list.add("山东大学"+Tool.yearOfDate()+"年度优秀社会实践报告（三等奖）");
        for (int i=0;i<list.size();i++) {
        	 Honor h = new Honor();
             h.setHonorType(5);//个人申报
             h.setActivityId(form.getId());
             h.setHonorName((String)list.get(i));
             h.setIsEnable(1);
        ...
        list.clear();
        list.add("山东大学"+Tool.yearOfDate()+"年度优秀社会实践团队");
        list.add("山东大学"+Tool.yearOfDate()+"年度优秀社会实践报告（一等奖）");
        list.add("山东大学"+Tool.yearOfDate()+"年度优秀社会实践报告（二等奖）");
        list.add("山东大学"+Tool.yearOfDate()+"年度优秀社会实践报告（三等奖）");
        for (int i=0;i<list.size();i++) {
       	 Honor h = new Honor();
            h.setHonorType(6);//集体申报
            h.setActivityId(form.getId());
            h.setHonorName((String)list.get(i));
            h.setIsEnable(1);
            h.setAddperson(userInfo.getId()+"");
...
forwardMap.put("form", "PracticeForm.jsp");
...
if (forwardKey.equals("list")) {
	...
	System.out.println((String)forwardMap.get(forwardKey) + "?cmd=" + cmd + "&page=" + currpage);
