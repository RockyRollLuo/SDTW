...
<%
	...
	int Pcurrpage = ParamUtils.getIntParameter(request, "Ppage", 1);//上一级页
		...
		<script>
			...
			function modifyRecord1(className, id, currpage, url_para, cmd) {
			    ...
			    var new_url = url_para;
			   	window.location=  "ClubActivitiesAction.jsp?cmd="+cmd+"&" + keyfield + "=" + id + "&page=" + currpage + ((new_url.length == 0) ? "" : "&" + new_url);
			}
			function returnP(url_para){
				var new_url = url_para;
				var currpage = '<%=Pcurrpage%>';
				window.location= "ClubActivityProjectAction.jsp?page=" + currpage + ((new_url.length == 0) ? "" : "&" + new_url);
			}
		</script>
	</head>
	<body onload="init();">
						...
						<tr>
							<td valign="top">
                                   ...
                                   <td>
											            <div class="browsetitle title_css">
												            <div class="browsetitleleft">
													          <div class="browsetitle001">活动立项</div>
													          <div class="browsetitle002"><%=XtUtil.getSpell("活动立项")%></div>
												          </div>
												          <div class="browsetitleright">
												              <div class="anniulist2">
													           <ul>
														            ...
														            <li><a href="javascript:returnP(url_para)">返回</a></li>
                               ...
                               <div class="browsetable2">
											<input type="hidden" name="cmd" value="clubproject">
											<input id="ProjectId" name="ProjectId" value="<%=ParamUtils.getIntParameter(request,"ProjectId",-1)%>" type="hidden"/>
											<%=Tool.join("\n", forms)%>
                               </div>
										</td>
									</tr>
