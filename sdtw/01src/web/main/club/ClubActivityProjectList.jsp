	...
	<head>
		...
		<script>
			...
			function showList1(className, currpage, url_para, aHasCheckbox, cmd, haveSelect,curId,showEdit) {
			    ...
			    if(userInfo.hasFunPower(Power.STUSER)){%>
			    if(showEdit == "1"){
			    	str = str + "<td bgcolor=\"#3992d0\" nowrap><div class=\"browsetable004\">查看项目</div></td><td bgcolor=\"#3992d0\" nowrap><div class=\"browsetable004\">参与立项</div></td>\n";
			    }
			    <%}
			    %>
			    <%
			    ...
			    if(userInfo.hasFunPower(Power.ADMINPOWER)||userInfo.hasFunPower(Power.XSST))desc = "编辑";
			    %>
			    if(showEdit == "1"){
			    	str = str + "<td bgcolor=\"#3992d0\" nowrap><div class=\"browsetable004\"><%=desc%></div></td><td bgcolor=\"#3992d0\" nowrap><div class=\"browsetable004\">参看参与立项活动</div></td>\n";
			    }
			    <%}
			    %>
			  ...
			  /*  if(showEdit == "1"){
			    ...
			    str = str + "</tr>\n";
			    if (rows.length == 0) {
			        str = str + "<tr><td colspan=" + (showfields.length + 3) + " bgcolor=\"#FFFFFF\" ><div class=\"browsetable006\">没有记录！</div></td></tr>\n";
			    } 
			    else {
			        for (i = 0; i < rows.length; i ++) {
			            ...
			            if(showEdit == "1"){
			            	...
			            	if(rows[i][13]==='1'){
			            	str = str + "<td class=\"xxybgx2\"  onclick=\"javascript:modifyRecord1_show('" + rows[i][1] + "','" + rows[i][keyindex] + "')\">&nbsp;</td><td class=\"xxybgx2\" onclick=\"javascript:Apply('" + rows[i][keyindex] + "','" + currpage + "','" + url_para + "')\"></td>";
			            	}else{
			            		str = str + "<td class=\"xxybgx2\"  onclick=\"javascript:modifyRecord1_show('" + rows[i][1] + "','" + rows[i][keyindex] + "')\">&nbsp;</td><td style=\"cursor: pointer;\"  onclick=\"javascript:Apply('" + rows[i][keyindex] + "','" + currpage + "','" + url_para + "')\">已结束，查看</td>";
			            	}
			            	<%}
			            	%>
			            	<%
			            	if(userInfo.hasFunPower(Power.ADMINPOWER)||userInfo.hasFunPower(Power.XSST)){%>
			            	str = str + "<td class=\"xxybgx2\"  onclick=\"javascript:modifyRecord1('" + className + "','" + rows[i][keyindex] + "','" + currpage + "','" + url_para + "','" + cmd + "')\">&nbsp;</td><td class=\"xxybgx2\" onclick=\"javascript:Apply('" + rows[i][keyindex] + "','" + currpage + "','" + url_para + "')\"></td>";
			            	<%}%>
			            }
			             var atitle = "";
			            for (var j = 0; j < fields.length; j ++) {
			                var k = showfields[j];
			                if(allfields[k]=="Id" ||allfields[k]=="OrderNo" ||allfields[k]=="ProjectName" || allfields[k]=="Subject" || allfields[k] == "Email"|| allfields[k] == "Title"){
			                    ...
			                    if(j==0)
			                		str = str + getRow1("<a class=visit style='cursor:hand' href=javascript:modifyRecord1('" + rows[i][1] + "','" + rows[i][keyindex] + "'); title='" + atitle + "'>"+rows[i][k]+"</a>", fields[j]);
			                	else
			                		str = str + getRow("<a class=visit style='cursor:hand' href=javascript:modifyRecord1('" + rows[i][1] + "'); title='" + atitle + "'>"+rows[i][k]+"</a>", fields[j]);
			                }else{
			                	if(j==0)
			...
			function modifyRecord1_show(title,id){
				parent.showDialog('<%=HeadConst.apache_url%>/main/club/ClubActivityProject.jsp?id='+id,600,600,title
						,{});
			}
			function Apply(id,currpage, url_para) {
				var className = "ClubActivities";
				var cmd = "project";//立项
			    ...
			    var new_url = url_para;
			    var keyfield = "ProjectId";
			    <%
			    //社团用户
			    if(userInfo.hasFunPower(Power.STUSER)){%>
			   		window.location= "ClubActivitiesAction.jsp?cmd="+cmd+"&" + keyfield + "=" + id;
			    <%}
			    %>
			    <%
			    if(userInfo.hasFunPower(Power.ADMINPOWER)||userInfo.hasFunPower(Power.XSST)||userInfo.hasFunPower(Power.UserAdminPower)){%>
			    window.location= "ClubActivitiesAction.jsp?_Module_=club-project&cmd=clubproject&" + keyfield + "=" + id+"&Ppage=" + currpage + ((new_url.length == 0) ? "" : "&" + new_url);
			    <%}
			    %>
			}
		</script>
