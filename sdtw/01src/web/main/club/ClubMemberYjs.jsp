...
cdt1.add("flag=1");
List AcademyNameOptions = COptionConst.getAcademyNameOptions(userInfo,"",cdt1);
...
cdt2.add("academy=-1");
List MajorNameoptions = COptionConst.getMajorNameOptions(userInfo,"",cdt2);
%>
<html>
<head>
...
function submitthis() {
	...
	if (MemberName==""||MemberName=="-1") {
		alert("请录入姓名");
		$('#MemberName').focus();
		return;
	}
	var MemberSno = $('#MemberSno').val();
	if (MemberSno==""||MemberSno=="-1") {
		$('#MemberSno').focus();
		alert("请录入学号");
		return;
	}
	var MemberAcademy = $('#MemberAcademy').val();
	if (MemberAcademy==""||MemberAcademy=="-1") {
		$('#MemberAcademy').focus();
		alert("请选择学院");
		return;
	}
...
<input id="Identity" name="Identity" value="<%=Identity%>" type="hidden"/>
	<table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" >
		...
		<tr>
			...
			<td align="left"  bgcolor="#ffffff">
			<input id="MemberSno" name="MemberSno"/>
			</td>
			<td align="right" class="form_label">学院</td>
			<td align="left"  bgcolor="#ffffff">
			<%=HtmlTool.renderSelect(AcademyNameOptions,"","MemberAcademy","","onchange=getmajor()") %>
			</td>
			<td align="right" class="form_label">专业</td>
			<td align="left"  bgcolor="#ffffff">
			<%=HtmlTool.renderSelect(MajorNameoptions,"","MemberMajor","") %>
			</td>
		</tr>
		<tr>
		<td align="right" class="form_label">生日</td>
		<td align="left"  bgcolor="#ffffff">
		<input readonly="readonly" class= "date" name ="MemberBrith" id= "MemberBrith" onfocus="WdatePicker({el:'MemberBrith',dateFmt:'yyyy-MM-dd',startDate: '%y-%M-%d'})"  size= "0"/>
		</td>
