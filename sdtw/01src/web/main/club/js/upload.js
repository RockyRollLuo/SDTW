	function uploadOtherAttatch(AttachType,FromClass){//上传其他附件
			    ...
			    (
			        {
			            url:'../../main/common/upload.jsp?cmd=otherattach&AttachType='+AttachType+'&FromTable='+FromClass,
			            secureuri:false,
			            ...
			            success: function (data, status)
			            {
			                if(typeof(data.error) != 'undefined')
			                {
			                    ...
			                    {
			                    	 if(document.postForm.BelongId.value==""){
			                         	var be = data.url.indexOf("=");
			                         	var en = data.url.lastIndexOf("/");
			                         	document.postForm.BelongId.value = data.ids;
			                         }else{
			                            var be = data.url.indexOf("=");
			                         	var en = data.url.lastIndexOf("/");
			                         	document.postForm.BelongId.value += ","+data.ids; 
						...
						success:function(data, status, e){
				        	if(typeof(data.error) != 'undefined'){
			                    ...
			                    }else
			                    {
			        				...
			        				var files = document.postForm.BelongId.value;
			        				...
			        				if(files.indexOf(id)!=-1){
			        					...
			        					document.postForm.BelongId.value = files;
			        				}
			                    }
			                }
			             },
