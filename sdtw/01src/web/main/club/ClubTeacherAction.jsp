...
private String[] ListFields = {"ClubName","TeaName", "TeaAcademy", "TeaTel", "TeaPhone", "TeaEmail", "StartDate", "StopDate", "Generation"};
...
private List getListRows(javax.servlet.http.HttpServletRequest request, ClubTeacher pv, List cdt)
{
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        //row.add((String)UserCnNameMap.get(v.getTeaName()));
        ...
        if(v.getStartDate().getTime()==Tool.stringToDate("0001-01-01").getTime()){
        	row.add("未设置");
        } else {
        row.add(Tool.stringOfDate(v.getStartDate()));
        }
        if(v.getStopDate().getTime()==Tool.stringToDate("0001-01-01").getTime()){
        	row.add("未设置");
        } else {
       row.add(Tool.stringOfDate(v.getStopDate()));
        	
        }
        row.add((String)YesNoMap.get("" + v.getGeneration()));
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, ClubTeacher pv, boolean isAll)
{
    ...
    qval = ParamUtils.getParameter(request,"ClubName","-1");
    if(qval.length()>0&&!qval.trim().equals("-1"))
    cdt.add("ClubName="+qval);
    qval = ParamUtils.getParameter(request,"_ClubName_","-1");
    if(qval.length()>0&&!qval.trim().equals("-1"))
    cdt.add("ClubName="+qval);
    cdt.add("checkflag=1");
    if (!isAll) {
