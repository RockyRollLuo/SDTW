	...
	<head>
		...
		<script>
			...
			function showList1(className, currpage, url_para, aHasCheckbox, cmd, haveSelect,curId,showEdit) {
			    ...
			    %>
			    str = str+"<td bgcolor=\"#3992d0\" nowrap><div class=\"browsetable004\">各学院上传资料</div></td>";
			    <%} else {%>
			    str = str+"<td bgcolor=\"#3992d0\" nowrap><div class=\"browsetable004\">上传资料</div></td>";
			    <%}%>
			            ...
			            for (var j = 0; j < fields.length; j ++) {
			                ...
			                if(allfields[k]=="Id" ||allfields[k]=="OrderNo" ||allfields[k]=="Name" || allfields[k]=="Subject" || allfields[k] == "Email"|| allfields[k] == "Title"){
			                	...
			                	if(j==0)
			                		...
			                		str = str + getRow(rows[i][k], fields[j])+"<td onclick=inList("+rows[i][0]+")>点击进入</td>";
			                	}
			                }                
			            }        
			            str = str + "</tr>\n";
			...
			function inList(sid){
				<%
				if (userInfo.hasFunPower(Power.ADMINPOWER)||userInfo.hasFunPower(Power.ZZJS)) {%>
					location = "AcademyList.jsp?sid="+sid;
				<%}else{%>
				    location = "shangbaoList.jsp?sid="+sid+"&aid=<%=userInfo.getUserCode().getDeptId()%>";
				<%}
				%>
