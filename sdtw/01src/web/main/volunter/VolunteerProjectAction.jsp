...
private static final String[] allFormNames = {"cmd", "page", "idlist", "Id", "ProjectName", "Text", "StartTime", "EndTime", "Deadline", "Addaccount", "Addacademy", "AddFileList", "AddTime","baoming","rongyufenpei","rongyuguanli"};
private String[] DicKeys = {"Id", "ProjectName", "Text", "StartTime", "EndTime", "Deadline", "Addaccount", "Addacademy", "AddFileList", "AddTime","baoming","rongyufenpei","rongyuguanli"};
private String[][] DicValues = {{"int", "Id", "0", "hidden", ""}, {"string", "比赛名称", "45", "text", ""}, {"string", "详情介绍", "2000", "hidden", ""}, {"date", "起止日期", "0", "text", ""}, {"date", "结束日期", "0", "text", ""}, {"date", "报名截止日期", "0", "text", ""}, {"string", "添加账号", "45", "text", ""}, {"string", "申办单位", "45", "AcademyName", ""}, {"string", "上传文件列表", "500", "hidden", ""}, {"date", "添加时间", "0", "hidden", ""}, {"string", "报名管理", "45", "text", ""}, {"string", "荣誉分配", "45", "text", ""}, {"string", "荣誉奖项设置", "45", "text", ""}};
private String KeyField = "Id";
private String[] AllFields = {"Id", "ProjectName", "Text", "StartTime", "EndTime", "Deadline", "Addaccount", "Addacademy", "AddFileList", "AddTime","baoming","rongyufenpei","rongyuguanli"};
private String[] ListFields = {"ProjectName",  "Addacademy", "StartTime", "Deadline",  "baoming","rongyufenpei","rongyuguanli"};
private String[] ListFields1 = {"ProjectName",  "Addacademy", "StartTime", "Deadline",  "baoming"};
...
private List getListRows(javax.servlet.http.HttpServletRequest request, VolunteerProject pv, List cdt)
{
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        row.add("<a href='VolunteerProjectParticipateAction.jsp?cmd=list&ProjectId="+v.getId()+"' >点击进入</a> ");
   
        if(v.getEndTime().getTime()<new Date().getTime()){
        row.add("<a href='VolunteerProjectParticipateAction.jsp?cmd=list&&_CheckFlag_=1&ProjectId="+v.getId()+"' >点击进入</a> ");
        }else{
        	row.add("比赛未结束");
        }
        row.add("<a href='HonorAction.jsp?cmd=list&_honourType_=4&_ActivityId_="+v.getId()+"&_addacunt_="+v.getAddaccount()+" '>点击设置</a> ");
        rows.add(row);
    }
    return rows;
}
...
private void setList(javax.servlet.http.HttpServletRequest request, boolean isAll)
{
    ...
    request.setAttribute("fields", ListFields);}else{
	...
	if(!FileTool.exists(HeadConst.root_file_path+"/main/upload/"+c.get(Calendar.YEAR))){
	 ...
	 if(fl==-1){//上传失败
		...
		try{
			...
			if(attach.insert()>0){
				System.out.println("1122211进入文件上传");
				out.clear();
