...
<%@ page import="java.io.*,java.util.*,jxl.*,jxl.write.*,java.math.BigDecimal"%><%@page import="com.xietong.software.base.Power"%>
<%!
...
private String[] ListFields = { "Name","CnName","DeptId","Grade", "Gender", "MobilePhone", "Email"};
...
private List getListRows(javax.servlet.http.HttpServletRequest request, BaseUserCode pv, List cdt)
{
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        row.add(gradenamemap.get(v.getGrade()));
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, BaseUserCode pv, boolean isAll)
{
    ...
    if(userInfo.hasFunPower(Power.UserAdminPower)){
    	cdt.add("Deptid="+user.getDeptId());
    }
    qval = ParamUtils.getParameter(request, "_VolunteerfFag_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("volunteerfFag =" + qval  );
    }
    ...
    qval = ParamUtils.getParameter(request, "_Sacademy_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0) {
        cdt.add("Deptid="+ qval);
    }
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    {  
    	...
    	cdt.add("id="+id);
        BaseUserCode user= new BaseUserCode();
        user.setVolunteerfFag("1");
        user.updateByCondition(cdt);
        return mkRtn("update", "input1", form);
    }
    ...
    {   
        ...
        BaseUserCode user= new BaseUserCode();
        list= user.query(cdt);
        System.out.println(list.size()+"AAAAAAAAAAAAAAAAAA");
      user =(BaseUserCode)list.get(0);
        setForm(request, user);
        return mkRtn("update", "input", user);
    }
...
forwardMap.put("input1", "../../volunteer/volunteer-activity-list.jsp");
