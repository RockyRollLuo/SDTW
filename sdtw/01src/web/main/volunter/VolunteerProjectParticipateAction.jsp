...
private List getListRows(javax.servlet.http.HttpServletRequest request, VolunteerProjectParticipate pv, List cdt)
{
    ...
    cdt1.add("honorType =4");
    list1=honor.query(cdt1);
    if(list1.size()>0){
    	for(int i=0;i<list1.size();i++){
    		honor=( Honor)list1.get(i);
    		honormap.put(honor.getActivityId()+"A"+honor.getId(), honor.getHonorName());
    	}
    }
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        row.add((String)honormap.get(v.getProjectId()+"A"+v.getHonour()));
        row.add("" + v.getSubmitFlag());
        row.add(v.getCheckFlag()==0?"未审核":(v.getCheckFlag()==1?"审核通过":"<font color='red'>审核未通过<font>"));   
        row.add(v.getXycheckFalg()==0?"未审核":(v.getXycheckFalg()==1?"审核通过":"<font color='red'>审核未通过<font>"));  
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, VolunteerProjectParticipate pv, boolean isAll)
{
    ...
    qval = ParamUtils.getParameter(request, "_CheckFlag_", "-1");
    QueryValues.add(qval);
    
    if (qval.trim().length() > 0&& !qval.equals("-1")) {
        cdt.add("checkflag=" + qval);
    }
    qval = ParamUtils.getParameter(request, "ProjectId", "");
    QueryValues.add(qval);
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    {   
        ...
        VolunteerProjectParticipate vp=new VolunteerProjectParticipate();
        vp=vp.getById(id);
        String aHonour=ParamUtils.getParameter(request, "Honour","-1");
       vp.setHonour(aHonour);
       vp.update();
        return mkRtn("list", "list", form);
    }
...
if (forwardKey.equals("list")) {
   ...
   String projectid = ParamUtils.getParameter(request, "ProjectId", "");
    out.println("<script>window.location='VolunteerProjectParticipateAction.jsp?cmd=list&ProjectId="+projectid+"&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
