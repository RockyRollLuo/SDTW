...
private List getListRows(javax.servlet.http.HttpServletRequest request, Places_Applytable pv, List cdt)
{
    ...
    Map xueyuanoptions=CEditConst.getAcademyNameMap(userInfo);
    Map auimaps= CEditConst.getAuditMap(userInfo);
    //查询符合条件的资料
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        ...
        row.add(v.getPlaceApplycode());
        row.add(Tool.jsSpecialChars(v.getApplyDate()));
        row.add(v.getApplyTimeRange());
        row.add(v.getApplicant());
        row.add( v.getActivityPrincipal());
        ...
        row.add((String)auimaps.get(v.getStatus()));
...
private List getListRows_forexcel(javax.servlet.http.HttpServletRequest request, Places_Applytable pv, List cdt)
{
	UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    Map UserNamemap = CEditConst.getUserNameMap(userInfo);
...
private void writeExcel(HttpServletRequest request, String filename) {
    UserInfo userInfo = Tool.getUserInfo(request);
    String[][] excelValues = { {"string", "场地名称", "45", "AcademyName", ""}, {"string", "申请时间", "45", "text", ""}, {"string", "申请时间段", "45", "text", ""}, {"string", "使用单位", "45", "text", ""}, {"string", "活动负责人", "45", "text", ""},{"string", "活动负责人职务", "45", "text", ""}, {"string", "活动负责人联系电话", "45", "hidden", ""}, {"date", "活动时间", "0", "hidden", ""}, {"string", "审核结果", "45", "text", ""}};
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("excel"))
    {
        ...
        System.out.println("1111111111111222221");
    ...
    if (cmd.equals("save"))
    {String shujus =ParamUtils.getParameter(request, "applydatastr", "");
	System.out.println(shujus+"11111111111111111111111111");
	String [] sapplydata=Tool.split(",,",shujus);
	String apply="";
	for(int i=0;i<sapplydata.length;i++){
		apply=sapplydata[i];
		String [] sapply=Tool.split(",",apply);
		form.setPlaceApplycode(sapply[0]);
		form.setApplyDate(sapply[1]);
		form.setApplyTimeRange(sapply[2]);
		  form.insert();
	}
        form.insert();
        return mkRtn("list", "list", form);
    ...
    if (cmd.equals("modifylist"))
    {
        ...
        cdt.add("id in (" + idlist + ")");
        Places_Applytable vp=new Places_Applytable();
