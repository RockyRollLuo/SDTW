 ...
 * $Id: validation-framework.js,v 1.6 2017/04/21 02:20:59 zhangguolong Exp $
 */
...
function validateUeditor(fform){
...
function doValidate(formRef) {
	try {
		...
		if (form != null) {
			var re = ValidationFramework.validateForm(form);
//			if(re){
//				re = validateUeditor(form);
//			}
			return re;
		} else {
			return false;
...
ValidationFramework.validateDepend = function(field, depend) {
	...
	} else if (depend.getName() == "code") {
		return ValidateMethodFactory.validateCode(field, depend.getParams());
	...
	} else if (depend.getName() == "equalsLength") {
		return ValidateMethodFactory.validateEqualsLength(field, depend.getParams());
	...
	} else if (depend.getName()=="notselected") {
		return ValidateMethodFactory.validateNotSelected(field, depend.getParams());
	} else if (depend.getName()=="fixedlengthinteger"){
		return ValidateMethodFactory.validateFixedLengthInteger(field, depend.getParams());
	}else {
...
ValidationFramework.getDocumentElement = function() {
	...
	/* 修改了不支持chrome的情况 by Yyb chrome没有实现xmldocument.load方法，只能使用xmlhttprequest方法获得xml文件
...
ValidationFramework.getErrorString = function(field, depend) {
	...
	//add  by dyh 2011-11-17 自定义错误提示
	if(field.getOnFail()!=null){
		retStr = field.getOnFail();
		return retStr;
	}
	
	retStr = retStr.replace("{0}", field.getDisplayName());
	if (dep == "minlength" || dep == "maxlength" || dep == "equalslength" || dep == "date" || dep == "fixedlengthinteger") {
...
ValidateMethodFactory.validateCode = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
...
ValidateMethodFactory.validateEqualsLength = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	var v = ValidationFramework.getIntegerValue(params[0]);
	return (obj.value.length == v);
...
ValidateMethodFactory.validatePhone = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	return ValidateMethodFactory.__checkPhone(obj.value);
}
 ...
 * 验证下拉框是否选中
 */
ValidateMethodFactory.validateNotSelected = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	return (!(obj.value==-1||obj.value==""));
}
/**
 * 验证定长整数
 */
ValidateMethodFactory.validateFixedLengthInteger = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	var v = ValidationFramework.getIntegerValue(params[0]);
	if (obj.value.length != v) return false;
	var exp = new RegExp("^([-]?\\d+)$");
	return exp.test(obj.value);
}
/**
 ...
 */
ValidateMethodFactory.__checkPhone= function(phoneStr) {     
	 ...
	 var myreg=/^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7})+(\-[0-9]{1,4})?$|(^(13[0-9]|15[0|3|6|7|8|9]|18[8|9])\d{8}$)/;
      if(myreg1.test(phoneStr)||myreg.test(phoneStr)){
    	  return true;
      }
      return false;   
}
...
ValidationErrorString["zh-cn"]["equalslength"]="{0}长度必须是{1}个字符。" ;
...
ValidationErrorString["zh-cn"]["pid"]="{0}不是一个有效父ID。";
ValidationErrorString["zh-cn"]["email"]="{0}不是一个有效的Email。";
ValidationErrorString["zh-cn"]["phone"]="{0}手机或固话不合法，固话格式为0000（区号）-0000000";
ValidationErrorString["zh-cn"]["equalsfield"]="{0}必须和{1}一致。";
ValidationErrorString["zh-cn"]["notselected"]="请选择{0}。";
ValidationErrorString["zh-cn"]["fixedlengthinteger"]="{0}必须是一个{1}位整数";
////English(en-us)
...
ValidationErrorString["en-us"]["code"]="{0} must be common ascii characters, A-Z,a-z,0-9.";
ValidationErrorString["en-us"]["integer"]="{0} must be a integer.";
ValidationErrorString["en-us"]["double"]="{0} must be a double value. ";
ValidationErrorString["en-us"]["number"]="{0} must be a interger or double value. ";
...
ValidationErrorString["en-us"]["equalslength"]="{0} must be equals {1}. " ;
...
loadXML = function(fileRoute){//根据不同浏览器读取xml文件，by Yyb
    xmlDoc=null;
