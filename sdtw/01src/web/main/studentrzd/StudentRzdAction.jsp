...
private String[][] DicValues = {{"int", "Id", "0", "hidden", ""}, {"int", "学号", "0", "hidden", ""}, {"string", "学院", "0", "AcademyName", ""}, {"int", "类别", "0", "StudentRzdTypeName", ""}, {"int", "修改标志", "0", "hidden", ""}, {"string", "审核原因", "100", "text", ""}, {"int", "审核标志", "0", "hidden", ""}, {"int", "活动id", "0", "hidden", ""}, {"string", "名称", "45", "text", ""}, {"string", "单位", "45", "text", ""}, {"string", "时间", "45", "text", ""}, {"string", "成果", "45", "text", ""}};
...
private String[] ListFields = {"Deptid", "Stuid","Type", "Value1", "Value2", "Value3", "Value4","Checkflag"};
...
//     String ordertype = ParamUtils.getParameter(request, "ordertype", "desc");
    String orderfield = ParamUtils.getParameter(request, "orderfield", "Checkflag");
    String ordertype = ParamUtils.getParameter(request, "ordertype", "");
    ...
    String qval = "";
  	...
  	//学生提交了的
	cdt.add("modifyflag="+SdtwConst.RZDMODIFY_YTJ);  	
    ...
    if(_academy_.trim().length()>0&&!"".equals(_academy_)){
    	cdt.add("deptid="+_academy_);
    }
    //_type_
    String _type_=ParamUtils.getParameter(request, "_type_","");
    if(_type_.trim().length()>0&&!"".equals(_type_)){
    	cdt.add("type="+_type_);
    }
    //_sno_,由于存的是userid,需要通过sno找到userid
    String _sno_=ParamUtils.getParameter(request, "_sno_","");
    if(!"".equals(_sno_)){
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    {
        String idlist = ParamUtils.getParameter(request, "idlist", "-1");
        int checkflag=ParamUtils.getIntParameter(request, "checkflag",SdtwConst.RZDCHECK_WSH);
        List cdt = new ArrayList();
        StudentRzd sr=new StudentRzd();
        cdt.add("id in (" + idlist + ")");
        sr.setCheckflag(checkflag);
        sr.updateByCondition(cdt);
        return mkRtn("list", "list", form);
    }
