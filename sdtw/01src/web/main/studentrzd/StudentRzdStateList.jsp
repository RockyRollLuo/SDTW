...
%>
  ...
  /*  if(showEdit == "1"){
    ...
    str = str + "<td bgcolor=\"#3992d0\" nowrap><div class=\"browsetable004\">查看</div></td></tr>\n";
    if (rows.length == 0) {
        ...
        for (i = 0; i < rows.length; i ++) {
            ...
            str = str + "<td><a href=\"#\" onclick=\"viewFunc('"+rows[i][keyindex]+"')\">查看</a></td></tr>\n";
        }
    }
    str = str + '</table>';
	...
	function viewFunc(stateid){
		window.open("StudentRzd-print.jsp?cmd=view&stateid="+stateid,"_blank",
...
<!-- 														            <li class="browsetitle_lang"><a href="javascript:javascript:document.query.cmd.value = 'excel'; document.query.submit();">导出Excel</a></li> -->
														            <li><a href="StudentRzdAction.jsp">返回</a></li>
                               ...
                               <div class="browsetable2">
											...
											<%if(userInfo.hasFunPower(Power.ADMINPOWER)||userInfo.hasFunPower(Power.RZD)){%>
												学院：<%=HtmlTool.renderSelect(AcademyNameoptions,ParamUtils.getParameter(request, "_academy_",""), "_academy_","")%>
											<%}%>
											打印时间：<input type="text" name="_printtime_" value="<%=ParamUtils.getParameter(request, "_printtime_","")%>" placeholder="yyyy-mm-dd"/>
											学号：<input type="text" name="_stuNo_" value="<%=ParamUtils.getParameter(request, "_stuNo_","")%>"/>
											<%=Tool.join("\n", forms)%>
                               </div>
										</td>
									</tr>
