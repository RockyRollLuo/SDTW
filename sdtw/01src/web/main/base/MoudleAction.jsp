...
private void setTreeSelect(javax.servlet.http.HttpServletRequest request)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    String ids = ParamUtils.getParameter(request, "ids", "");
    String flag = ParamUtils.getParameter(request, "flag", "0");
    request.setAttribute("ids", ids);
    request.setAttribute("flag", flag);
    request.setAttribute("keyfield", KeyField);
    request.setAttribute("classname", "Moudle");
...
private void setForm(javax.servlet.http.HttpServletRequest request, BaseMoudle form)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    System.out.println(CEditConst.getMoudleOrFunOptions(userInfo, "-1"));
    request.setAttribute("MoudleOrFunoptions", CEditConst.getMoudleOrFunOptions(userInfo, "-1"));
    List cdt = new ArrayList();
    cdt.add("MoudleGrade=" + (form.getMoudleGrade()-1));
    request.setAttribute("MoudleNameoptions", COptionConst.getMoudleNameOptions(userInfo, "-1", cdt));
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("treeSelect"))
    {
    	setTreeSelect(request);
    	//增加日志
        return mkRtn("treeSelect", "treeSelect", null);
    }
...
forwardMap.put("list", "MoudleAction.jsp");
forwardMap.put("failure", "MoudleForm.jsp");
forwardMap.put("success", "MoudleList.jsp");
 
forwardMap.put("input", "MoudleForm.jsp");
forwardMap.put("tree", "MoudleTree.jsp");
forwardMap.put("treeSelect", "MoudleTreeSelect.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script>window.location='MoudleAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
}
else {
