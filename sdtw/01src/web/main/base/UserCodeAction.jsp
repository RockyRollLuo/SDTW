...
<%@page import="com.xietong.software.sdtw.db.Major"%>
...
private String[] ListFields = {"OrgId", "DeptId", "Name", "CnName", "Roles", "Grade", "Major","IsEnable"};
private String[] ListFieldsPrint = {"OrgId","DeptId" ,"Grade", "Major","Name", "CnName", "IDNum", "Roles","IsEnable"};
private String[] FormFields = {"OrgId", "DeptId", "Name", "CnName", "Code", "Gender", "Occupation", "Grade", "Dept", "Orderby", "Principal", "PhoneNum", "MobilePhone", "Fax", "OfficeAddress", "Email", "HomeAddress", "HomeZipcode", "DeptZipcode", "UserIpphome", "LastTime", "SkinId", "DispNum", "OrderbyField", "OrderStyle", "MsgRefreTime", "Privilege", "Roles", "EditStyle", "IsFirstLogon", "OperLog", "MdlPower"};
...
private BaseUserCode getByParameterDb(javax.servlet.http.HttpServletRequest request)
{
    ...
    if (cmd.equals("update")) {
       ...
       // v.setPwd(ParamUtils.getParameter(request, "Pwd", ""));
        v.setPwd(ParamUtils.getParameter(request, "Pwd", Tool.encrypt("123456")));
...
private List getListRows(javax.servlet.http.HttpServletRequest request, BaseUserCode pv, List cdt)
{
   ...
   Map YesNomap = CEditConst.getYesNoMap(userInfo);
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
         ...
         row.add("" + v.getId());
         row.add((String)OrgCityNamemap.get("" + v.getOrgId()));
         ...
         row.add(YesNoMap.get("" + v.getIsEnable()));
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, BaseUserCode pv, boolean isAll)
{
    ...
    qval = ParamUtils.getParameter(request, "_Major_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0&&!qval.equals("-1")) {
        cdt.add("major=" + qval);
    }
    ...
    qval = ParamUtils.getParameter(request, "_Roles_", "");
    QueryValues.add(qval);
    if (qval.trim().length() > 0&&!qval.equals("-1")) {
        cdt.add("roles=" + qval);
    }
   ...
   /*  if (cmd.equals("excel"))
    ...
    if (cmd.equals("save"))
    {
        ...
        if (isDuplicated(form, cmd)) {
            request.setAttribute("msg", "用户账号重复!");
            setForm(request, form);
            return mkRtn("save", "input", form);
        }
        else {
...
if (forwardKey.equals("list")) {
	...
	if (list.size()>0) {
		b.append("<option value='-1'></option>");
		for (int i=0;i<list.size();i++) {
			m = (Major)list.get(i);
			b.append("<option value="+m.getId()+">"+m.getName()+"</option>");
		}
	}
	out.print(b.toString());
	return;
} else if ("setGraduate".equals(tmp)) {
	//cmd:'setGraduate',academy:academy,major:major,grade:grade
	int academy = ParamUtils.getIntParameter(request,"academy",-1);
	int major = ParamUtils.getIntParameter(request,"major",-1);
	int grade = ParamUtils.getIntParameter(request,"grade",-1);
	List cdt = new ArrayList();
	cdt.add("major="+major);
	cdt.add("DeptId="+academy);
	cdt.add("grade="+grade);
	BaseUserCode user = new BaseUserCode();
	user.setIsEnable(0);
	user.setOperTime(new Date());
	user.setOperLog(user.getOperLog()+" 在"+Tool.stringOfCnDateTime(new Date())+" 用户"+userInfo.getName()+" 设置该学生毕业");
	int num = user.updateByCondition(cdt);
	if (num>0) {
		out.print("{\"msg\":\"ok\"}");
		return;
	} else {
