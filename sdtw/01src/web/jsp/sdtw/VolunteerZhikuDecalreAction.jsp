...
<%!
private static Log log = LogFactory.getLog(VolunteerZhikuDecalre.class);
...
private String[][] DicValues = {{"int", "id", "0", "hidden", ""}, {"int", "志库通知的id", "0", "hidden", ""}, {"string", "添加账号", "20", "hidden", ""}, {"date", "添加时间", "0", "hidden", ""}, {"string", "学院", "45", "academy", ""}, {"int", "审核标志", "0", "hidden", ""}, {"string", "上传文件", "500", "hidden", ""}, {"string", "项目名称", "45", "text", ""}, {"string", "团队名称", "45", "text", ""}, {"string", "所属单位", "45", "text", ""}, {"string", "项目分组", "45", "zhikuType", ""}, {"string", "项目活动背景及目的", "500", "hidden", ""}, {"string", "活动流程", "500", "hidden", ""}, {"string", "预期成果", "500", "hidden", ""}, {"string", "预期成果", "500", "hidden", ""}, {"string", "已取得成果", "500", "hidden", ""}, {"string", "（项目情况）交通费", "45", "text", ""}, {"string", "（项目情况）住宿费", "45", "text", ""}, {"string", "（项目情况）其他1", "45", "text", ""}, {"string", "（项目情况）其他2", "45", "text", ""}, {"string", "（项目情况）其他3", "45", "text", ""}, {"string", "（项目情况）其他4", "45", "text", ""}, {"string", "（项目情况）总计", "45", "text", ""}};
...
private String[] ListFields = {"Academy", "Name", "Teamname", "Unit", "Type", "Cost1", "Cost2", "Cost3", "Cost4", "Cost5", "Cost6", "Totalcost"};
private String[] FormFields = {"Academy", "Name", "Teamname", "Unit", "Type", "Intention", "Arrange", "Process", "Expectresults", "Results", "Cost1", "Cost2", "Cost3", "Cost4", "Cost5", "Cost6", "Totalcost"};
private String[] QueryFields = {""};
private String[] HideFields = {"Id"};
private VolunteerZhikuDecalre getByParameterDb(javax.servlet.http.HttpServletRequest request)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    VolunteerZhikuDecalre v = new VolunteerZhikuDecalre();
...
private List getListRows(javax.servlet.http.HttpServletRequest request, VolunteerZhikuDecalre pv, List cdt)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    //查询符合条件的资料
    ...
    for (Iterator it = vs.iterator(); it.hasNext();) {
        VolunteerZhikuDecalre v = (VolunteerZhikuDecalre)it.next();
        List row = new ArrayList();
        row.add("" + v.getId());
        row.add("" + v.getZhikuId());
        ...
        row.add(Tool.jsSpecialChars(v.getUnit()));
...
private void setListData(javax.servlet.http.HttpServletRequest request, VolunteerZhikuDecalre pv, List cdt)
{
    List rows = new ArrayList();
...
private List getListCondition(javax.servlet.http.HttpServletRequest request, VolunteerZhikuDecalre pv, boolean isAll)
{
    UserInfo userInfo = Tool.getUserInfo(request);
...
{
    UserInfo userInfo = Tool.getUserInfo(request);
    VolunteerZhikuDecalre pv = new VolunteerZhikuDecalre();
    ...
    request.setAttribute("classname", "VolunteerZhikuDecalre");
...
private void setForm(javax.servlet.http.HttpServletRequest request, VolunteerZhikuDecalre form)
{
    UserInfo userInfo = Tool.getUserInfo(request);
    //默认值定义
    request.setAttribute("dickeys", DicKeys);
...
private List mkRtn(String cmd, String forward, VolunteerZhikuDecalre form)
{
...
public List main(javax.servlet.http.HttpServletRequest request)
{
    ...
    String cmd = ParamUtils.getParameter(request, "cmd", "list");
    log.debug("cmd=" + cmd);
    VolunteerZhikuDecalre form = getByParameterDb(request);
    if (cmd.equals("list"))
    {
        setList(request);
...
log.debug("VolunteerZhikuDecalreAction");
int currpage = ParamUtils.getIntParameter(request, "page", 1);
List rtn = null;
UserInfo userInfo = Tool.getUserInfo(request);
if (userInfo==null) {
...
forwardMap.put("list", "VolunteerZhikuDecalreAction.jsp");
forwardMap.put("failure", "VolunteerZhikuDecalreForm.jsp");
forwardMap.put("success", "VolunteerZhikuDecalreList.jsp");
forwardMap.put("excel", "/upload/temp/" + userInfo.getName() + ".xls");
forwardMap.put("input", "VolunteerZhikuDecalreForm.jsp");
...
if (forwardKey.equals("list")) {
    ...
    out.println("<script>window.location='VolunteerZhikuDecalreAction.jsp?cmd=list&page=" + currpage + ((url.length() == 0) ? "" : "&" + url) + "';</script>");
}
else {
